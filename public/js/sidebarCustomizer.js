import p from"./api.js";import{Toast as b,Modal as i,BottomSheet as l}from"./ui.js";const c=[{key:"dashboard",href:"#/dashboard",label:"Dashboard",icon:"\u{1F4CA}",isEnabled:!0},{key:"students",href:"#/students",label:"Students",icon:"\u{1F393}",isEnabled:!0},{key:"seats",href:"#/seats",label:"Seats",icon:"\u{1FA91}",isEnabled:!0},{key:"plans",href:"#/plans",label:"Plans",icon:"\u{1F4E6}",isEnabled:!0},{key:"lockers",href:"#/lockers",label:"Lockers",icon:"\u{1F510}",isEnabled:!0},{key:"payments",href:"#/payments",label:"Payments",icon:"\u{1F4B3}",isEnabled:!0},{key:"attendance",href:"#/attendance",label:"Attendance",icon:"\u{1F4C5}",isEnabled:!0},{key:"shifts",href:"#/shifts",label:"Shifts",icon:"\u23F0",isEnabled:!0},{key:"branches",href:"#/branches",label:"Branches",icon:"\u{1F3E2}",isEnabled:!1},{key:"reports",href:"#/reports",label:"Reports",icon:"\u{1F4C8}",isEnabled:!0},{key:"expenses",href:"#/expenses",label:"Expenses (P&L)",icon:"\u{1F4B8}",isEnabled:!0},{key:"operations",href:"#/operations",label:"Operations",icon:"\u2699\uFE0F",isEnabled:!0},{key:"trash",href:"#/trash",label:"Recycle Bin",icon:"\u{1F5D1}\uFE0F",isEnabled:!0},{key:"settings",href:"#/settings",label:"Settings",icon:"\u2699\uFE0F",isEnabled:!0},{key:"profile",href:"#/profile",label:"My Profile",icon:"\u{1F464}",isEnabled:!0}],m={default:{name:"Standard Default",keys:["dashboard","students","seats","plans","lockers","payments","attendance","shifts","reports","expenses","operations","trash","settings","profile"]},frontdesk:{name:"Front Desk Essential",keys:["dashboard","students","seats","payments","attendance","settings","profile"]},all:{name:"All Modules Active",keys:["dashboard","students","seats","plans","lockers","payments","attendance","shifts","branches","reports","expenses","operations","trash","settings","profile"]}},g=["\u{1F4CA}","\u{1F393}","\u{1FA91}","\u{1F4E6}","\u{1F510}","\u{1F4B3}","\u{1F4C5}","\u23F0","\u{1F3E2}","\u{1F4C8}","\u{1F4B8}","\u2699\uFE0F","\u{1F5D1}\uFE0F","\u{1F6E0}\uFE0F","\u{1F464}","\u{1F4DA}","\u26A1","\u{1F4BC}","\u{1F4CC}","\u{1F514}","\u{1F3F7}\uFE0F","\u{1F3AF}"];class x{constructor(){this.items=[],this.modalInstance=null,this.userRole="staff",this.saveScope="global"}async open(){try{const t=localStorage.getItem("sl_user");if(t){const e=JSON.parse(t);this.userRole=e.role||"staff"}}catch{}if(typeof window<"u"&&!window.Sortable)try{const{loadSortable:t}=await import("./dragDrop.js");typeof t=="function"&&await t()}catch{}await this.loadConfig(),this.render()}async loadConfig(){try{const t=await p.get("/api/settings/sidebar/all");if(t&&t.success&&Array.isArray(t.data)&&t.data.length>0)this.items=t.data.map((e,s)=>({key:e.key,label:e.label||e.key,href:e.href||`#/${e.key}`,icon:this.formatIcon(e.icon,e.key),isEnabled:e.isEnabled!==!1,order:e.order!==void 0?e.order:s+1}));else{const e=await p.get("/api/settings/sidebar");if(e&&e.success&&Array.isArray(e.data)&&e.data.length>0){const s=new Set(e.data.map(r=>r.key)),o=new Map(e.data.map((r,a)=>[r.key,a+1]));this.items=c.map(r=>({...r,isEnabled:s.has(r.key),order:o.get(r.key)||99})).sort((r,a)=>r.order-a.order)}else this.items=JSON.parse(JSON.stringify(c))}}catch(t){console.warn("Could not fetch server sidebar, using defaults:",t.message),this.items=JSON.parse(JSON.stringify(c))}try{const t=localStorage.getItem("sl_sidebar_order_personal")||localStorage.getItem("sl_sidebar_order");if(t){const e=JSON.parse(t);if(Array.isArray(e)&&e.length>0){const s=new Map(e.map((o,r)=>[o.replace("#/",""),r+1]));this.items.forEach(o=>{s.has(o.key)&&(o.order=s.get(o.key))}),this.items.sort((o,r)=>o.order-r.order)}}}catch{}}formatIcon(t,e){if(!t||t.startsWith("<svg")){const s=c.find(o=>o.key===e);return s?s.icon:"\u{1F4CC}"}return t.trim()}render(){const t=["owner","branch_manager","admin"].includes(this.userRole),e=document.createElement("div");e.className="sidebar-customizer-wrap",e.style.cssText="display: flex; flex-direction: column; gap: 14px; max-height: 75vh; overflow-y: auto; padding-right: 4px;",e.innerHTML=`
      <style>
        .sc-module-row {
          display: flex;
          align-items: center;
    ...
    `,this.bindEvents(e),window.innerWidth<=768&&typeof l<"u"&&l.show?this.modalInstance=l.show({title:"\u{1F3A8} Customize Sidebar Navigation",content:e,height:"85vh"}):typeof i<"u"&&i.show&&(this.modalInstance=i.show({title:"\u{1F3A8} Customize Sidebar Navigation",content:e,size:"lg"}))}renderItemsHtml(){return this.items.map((t,e)=>{const s=e===0,o=e===this.items.length-1;return`
        <div class="sc-module-row ${t.isEnabled?"":"sc-disabled"}" data-key="${t.key}" data-index="${e}">
          ...
          <div style="display: flex; flex-direction: column; gap: 2px;">
            ...
            <button type="button" class="sc-reorder-btn sc-btn-down" data-index="${e}" ${o?"disabled":""} title="Move Down">\u25BC</button>
          </div>
      ...
      `}).join("")}refreshList(t){t.innerHTML=this.renderItemsHtml(),this.bindRowEvents(t)}bindEvents(t){const e=t.querySelector("#sc-items-container");t.querySelectorAll(".sc-preset-chip").forEach(s=>{s.addEventListener("click",()=>{const o=s.dataset.preset;this.applyPreset(o),this.refreshList(e)})}),t.querySelectorAll(".sc-scope-btn").forEach(s=>{s.addEventListener("click",()=>{this.saveScope=s.dataset.scope,t.querySelectorAll(".sc-scope-btn").forEach(o=>{o.className=`btn btn-xs ${o.dataset.scope===this.saveScope?"btn-primary":"btn-outline-secondary"} sc-scope-btn`})})}),t.querySelector("#sc-btn-reset")?.addEventListener("click",async()=>{confirm("Reset navigation layout to default factory order?")&&(this.items=JSON.parse(JSON.stringify(c)),this.refreshList(e))}),t.querySelector("#sc-btn-cancel")?.addEventListener("click",()=>{this.closeModal()}),t.querySelector("#sc-btn-save")?.addEventListener("click",async()=>{const s=t.querySelector("#sc-btn-save");s&&(s.disabled=!0);try{await this.saveChanges(),this.closeModal()}catch(o){b.error(o.message||"Failed to save navigation changes.")}finally{s&&(s.disabled=!1)}}),this.bindRowEvents(e)}bindRowEvents(t){if(typeof window<"u"&&window.Sortable&&t)try{this._sortableInst&&this._sortableInst.destroy(),this._sortableInst=window.Sortable.create(t,{handle:".sc-drag-handle",animation:180,ghostClass:"sortable-ghost",chosenClass:"sortable-chosen",onEnd:()=>{const e=Array.from(t.querySelectorAll(".sc-module-row")),s=new Map(e.map((o,r)=>[o.dataset.key,r]));this.items.sort((o,r)=>{const a=s.has(o.key)?s.get(o.key):999,n=s.has(r.key)?s.get(r.key):999;return a-n}),this.refreshList(t)}})}catch{}t.querySelectorAll(".sc-btn-up").forEach(e=>{e.addEventListener("click",()=>{const s=parseInt(e.dataset.index,10);if(s>0){const o=this.items[s];this.items[s]=this.items[s-1],this.items[s-1]=o,this.refreshList(t)}})}),t.querySelectorAll(".sc-btn-down").forEach(e=>{e.addEventListener("click",()=>{const s=parseInt(e.dataset.index,10);if(s<this.items.length-1){const o=this.items[s];this.items[s]=this.items[s+1],this.items[s+1]=o,this.refreshList(t)}})}),t.querySelectorAll(".sc-rename-input").forEach(e=>{e.addEventListener("input",()=>{const s=e.dataset.key,o=this.items.find(r=>r.key===s);o&&(o.label=e.value.trim())})}),t.querySelectorAll(".sc-toggle-switch").forEach(e=>{e.addEventListener("change",()=>{const s=e.dataset.key,o=this.items.find(r=>r.key===s);if(o){o.isEnabled=e.checked;const r=e.closest(".sc-module-row");r&&r.classList.toggle("sc-disabled",!e.checked)}})}),t.querySelectorAll(".sc-icon-btn").forEach(e=>{e.addEventListener("click",s=>{s.stopPropagation(),this.openEmojiPicker(e)})})}openEmojiPicker(t){const e=document.getElementById("sc-emoji-picker-dropdown");e&&e.remove();const s=document.createElement("div");s.id="sc-emoji-picker-dropdown",s.style.cssText=`
      position: fixed;
      z-index: 9999999;
      background: var(--color-surface, #fff);
      border: 1px solid var(--color-border, #cbd5e1);
      ...
      gap: 6px;
      max-width: 250px;
    `,g.forEach(a=>{const n=document.createElement("button");n.type="button",n.textContent=a,n.style.cssText="font-size: 1.25rem; border: none; background: transparent; cursor: pointer; border-radius: 6px; padding: 4px; transition: background 0.1s;",n.onmouseenter=()=>{n.style.background="rgba(108, 92, 231, 0.15)"},n.onmouseleave=()=>{n.style.background="transparent"},n.onclick=d=>{d.stopPropagation(),t.textContent=a;const y=t.dataset.key,u=this.items.find(f=>f.key===y);u&&(u.icon=a),s.remove()},s.appendChild(n)}),document.body.appendChild(s);const o=t.getBoundingClientRect();s.style.left=`${Math.min(window.innerWidth-260,Math.max(10,o.left))}px`,s.style.top=`${o.bottom+6}px`;const r=a=>{!s.contains(a.target)&&a.target!==t&&(s.remove(),document.removeEventListener("click",r))};setTimeout(()=>document.addEventListener("click",r),10)}applyPreset(t){const e=m[t];if(e)if(t==="all")this.items.forEach(s=>{s.isEnabled=!0});else{const s=new Set(e.keys);this.items.forEach(r=>{r.isEnabled=s.has(r.key)});const o=new Map(e.keys.map((r,a)=>[r,a]));this.items.sort((r,a)=>{const n=o.has(r.key)?o.get(r.key):999,d=o.has(a.key)?o.get(a.key):999;return n-d})}}async saveChanges(){const t=this.items.map((e,s)=>({key:e.key,label:e.label,href:e.href,icon:e.icon,isEnabled:e.isEnabled,order:s+1}));try{const e=t.filter(s=>s.isEnabled).map(s=>s.href);localStorage.setItem("sl_sidebar_order",JSON.stringify(e)),localStorage.setItem("sl_sidebar_order_personal",JSON.stringify(e)),localStorage.setItem("sl_sidebar_custom_items",JSON.stringify(t))}catch{}if(this.saveScope==="global"&&["owner","branch_manager","admin"].includes(this.userRole))try{const e=await p.put("/api/settings/sidebar",{items:t});if(!e||!e.success)throw new Error(e?.message||"Failed to update global sidebar")}catch(e){console.warn("Could not save globally, saved locally:",e.message)}b.success("Navigation layout updated successfully!"),typeof window.reloadSidebar=="function"?window.reloadSidebar():window.App&&typeof window.App.updateSidebarForRole=="function"&&window.App.updateSidebarForRole()}closeModal(){this.modalInstance&&(typeof this.modalInstance.close=="function"?this.modalInstance.close():typeof l<"u"&&l.close?l.close():typeof i<"u"&&i.closeAll&&i.closeAll(),this.modalInstance=null)}}const h=new x;window.SidebarCustomizer=h;var v=h;export{v as default};
