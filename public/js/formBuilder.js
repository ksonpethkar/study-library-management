var I=Object.defineProperty,D=(P,t,e)=>t in P?I(P,t,{enumerable:!0,configurable:!0,writable:!0,value:e}):P[t]=e,q=(P,t,e)=>D(P,typeof t!="symbol"?t+"":t,e);import b from"./api.js";import{Toast as u,Modal as k,Loading as F,Confirm as N,ActionMenu as A,escapeHTML as d}from"./ui.js";import"./mediaStudio.js";const _={text:"\u{1F4DD}",textarea:"\u{1F4C4}",number:"\u{1F522}",phone:"\u{1F4F1}",email:"\u{1F4E7}",date:"\u{1F4C5}",time:"\u23F0",select:"\u{1F4CB}",multiselect:"\u2611\uFE0F",radio:"\u{1F518}",checkbox:"\u2705",file:"\u{1F4CE}",photo_upload:"\u{1F4F8}",signature_pad:"\u270D\uFE0F",exam_badge:"\u{1F3AF}",blood_group:"\u{1FA78}",url:"\u{1F517}",color:"\u{1F3A8}",address_autocomplete:"\u{1F4CD}",aadhaar_pan:"\u{1FAAA}",terms_checkbox:"\u{1F4DC}",star_rating:"\u2B50"},B={personal:"\u{1F464}",academic:"\u{1F4DA}",plan:"\u{1F48E}",payment:"\u{1F4B3}",seat:"\u{1F4BA}",other:"\u{1F4DD}"},z=class m{static async render(t){this.container=t,this.sections=[],this.fields=[],this.branches=[],this.plans=[],this.seats=[],this.template={branding:{showLogo:!0,showBanner:!1,bannerImage:"",headerText:"Student Admission Wizard",tagline:"Silence, Focus & Success",alignment:"center",logoSize:"64"}},this.selectedBranchId=null,this.selectedPlanId=null,this.selectedSeatId=null,this.selectedPaymentMode="upi",this.currentPreviewStep=0,this.container.innerHTML=`
    ...
    `}static renderPreviewInput(t){const e=t.required?' <span style="color: var(--color-danger);">*</span>':"",req=t.required?"required":"",i=t.colSpan===6?"grid-column: span 1;":"grid-column: 1 / -1;",a=`${d(t.label)}${e}`,o=t.helpText?`<small class="text-muted" style="display:block; font-size:0.7rem; margin-top:3px;">${d(t.helpText)}</small>`:"";if(t.type==="select"){const r=(t.options||[]).map(l=>`<option>${d(l)}</option>`).join("");return`
        <div style="${i}">
          <label class="form-label text-xs" style="font-weight:600;">${a}</label>
          <select class="form-select form-control-sm" ${req}><option value="">-- Select --</option>${r}</select>
          ${o}
        </div>
      `}if(t.type==="radio"){const r=(t.options||[]).map((l,s)=>`
        <label style="font-size:0.8rem; margin-right:8px; display:inline-flex; align-items:center; gap:4px;">
          <input type="radio" name="prev_${t._id}" ${s===0?"checked":""} ${req}> ${d(l)}
        </label>
      `).join("");return`
        <div style="${i}">
          <label class="form-label text-xs" style="font-weight:600;">${a}</label>
      ...
      `}return t.type==="textarea"?`
        <div style="${i}">
          <label class="form-label text-xs" style="font-weight:600;">${a}</label>
          <textarea class="form-control form-control-sm" rows="2" placeholder="${d(t.placeholder||"")}" ${req}></textarea>
          ${o}
        </div>
      `:`
      <div style="${i}">
        <label class="form-label text-xs" style="font-weight:600;">${a}</label>
        <input type="${t.type==="number"?"number":t.type==="date"?"date":"text"}" class="form-control form-control-sm" placeholder="${d(t.placeholder||"")}" ${req}>
        ${o}
      </div>
...
`).map(n=>n.trim()).filter(Boolean);await b.put("/api/settings/profile",{rules:s})}catch{}}else t==="kiosk_barcode"&&(e.kioskBarcodeLabel=o.querySelector("#ce-label").value.trim(),e.kioskBarcodeSubtext=o.querySelector("#ce-subtext").value.trim(),e.showKioskBarcode=o.querySelector("#ce-active").checked);await b.put("/api/custom-fields/templates/active",{settings:e}),m.bustPublicFormCache(),u.success("Component settings saved successfully!"),r.close(),k.closeAll(),this.renderSections(),this.renderPreview()}catch(s){u.error(s.message||"Failed to save component settings")}})}static openFieldEditor(t,e=null){const i=this.fields.find(s=>s._id===t)||{label:"",fieldName:"",type:"text",section:e||"personal",required:!1,placeholder:"",helpText:"",colSpan:12,options:["Option 1","Option 2"]},a=!!t,o=this.sections.map(s=>`<option value="${s.name}" ${i.section===s.name?"selected":""}>${d(s.label)}</option>`).join(""),r=document.createElement("div");r.innerHTML=`
    ...
    `;const l=new k({title:a?`\u270F\uFE0F Edit Question: ${i.label}`:"\u2728 Create New Question Field",content:r,size:"md"});l.show(),r.querySelector(".fb-cancel-modal-btn")?.addEventListener("click",()=>{l.close(),k.closeAll()}),a||r.querySelector("#fe-label")?.addEventListener("input",s=>{const n=s.target.value.toLowerCase().replace(/[^a-z0-9_]/g,"_");r.querySelector("#fe-key").value=n}),r.querySelector("#fe-type")?.addEventListener("change",s=>{const n=s.target.value,c=["select","radio","multiselect"].includes(n),p=n==="conditional";r.querySelector("#fe-options-wrap").style.display=c?"block":"none",r.querySelector("#fe-conditional-wrap").style.display=p?"block":"none"}),r.querySelector("#fb-field-edit-form")?.addEventListener("submit",async s=>{s.preventDefault();const n=r.querySelector("#fe-label").value.trim(),c=r.querySelector("#fe-key").value.trim().toLowerCase().replace(/[^a-z0-9_]/g,"_"),p=r.querySelector("#fe-type").value,y=r.querySelector("#fe-section").value,v=r.querySelector("#fe-placeholder").value.trim(),g=r.querySelector("#fe-help").value.trim(),x=r.querySelector("#fe-required").checked,h=r.querySelector("#fe-halfwidth").checked?6:12,f=r.querySelector("#fe-options").value,w=f?f.split(",").map($=>$.trim()).filter(Boolean):[];let S=null,C=null;if(p==="conditional"){const $=r.querySelector("#fe-show-if-field")?.value,L=r.querySelector("#fe-show-if-op")?.value||"equals",T=r.querySelector("#fe-show-if-val")?.value.trim()||"";if(C=r.querySelector("#fe-conditional-subtype")?.value||"text",!$){u.error("Please pick a trigger field for the conditional logic.");return}S={field:$,operator:L,value:T}}const E={fieldName:c,label:n,type:p,section:y,placeholder:v,helpText:g,required:x,colSpan:h,options:w,isActive:!0,...S&&{showIf:S},...C&&{conditionalSubType:C}};try{a?(await b.put(`/api/custom-fields/${t}`,E),u.success("Question field updated successfully!")):(await b.post("/api/custom-fields",E),u.success("New question field added successfully!")),l.close(),await m.loadData()}catch($){u.error($.message||"Failed to save question field")}})}};q(z,"currentPreviewStep",0),q(z,"previewDeviceMode","desktop"),q(z,"undoStack",[]);let R=z;export{R as FormBuilder};
