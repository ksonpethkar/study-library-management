import"../app.js";import{t as Z}from"../i18n.js";import{Toast as k,Modal as ge,Loading as Ee,Confirm as Ne,escapeHTML as a,debounce as ft,copyToClipboard as nt,UI as Ae}from"../ui.js";import{SmartFormatters as pe}from"../utils/smartFormatters.js";import{SignatureStudio as yt}from"../signatureStudio.js";import{MediaFieldPicker as Re}from"../mediaStudio.js";import z from"../api.js";import{previewAdmissionFormPDF as je}from"../pdfGenerator.js";import{renderHeatmapGridHtml as ht}from"./portal.js";import{IDBStorage as ae}from"../utils/idbStorage.js";import{OptimisticUI as ot}from"../utils/optimisticUI.js";import{SmartIntelligence as be}from"../utils/smartIntelligence.js";import{renderHeatmap as vt,renderBehaviorBadge as xt}from"../utils/attendanceHeatmap.js";import{Validators as ue}from"../utils/validators.js";import{PaymentStudio as wt}from"../paymentStudio.js";import{initSwipeCards as $t}from"../utils/mobileGestures.js";import{ExportHelper as Ut}from"../utils/exportWorkerHelper.js";async function Ft(){const K=document.createElement("div");K.className="page-container";const Be=document.createElement("div");Be.className="module-header",Be.innerHTML=`
  ...
  `,Pe.appendChild(ze);const U=document.createElement("div");U.className="card-body p-0",U.id="students-table-container",Pe.appendChild(U),K.appendChild(Pe);const N={students:[],pagination:{page:1,limit:10,total:0,pages:1}};function Oe(e){xe.innerHTML=`
    ...
    `}async function fe(){let e=!1;try{const s=await ae.get("students","stats");s&&(Oe(s),e=!0)}catch(s){console.warn("IDB read stats warning:",s)}e||Ee.skeleton(xe,"kpi");try{const s=await z.get("/api/students/stats");if(s.success&&s.data){const r=s.data;await ae.set("students","stats",r),Oe(r)}}catch(s){console.error(s)}}async function ee(e=1){const s=K.querySelector("#studentSearch")?.value||"",r=K.querySelector("#studentStatusFilter")?.value||"all",w=`list_${e}_${s}_${r}`;let P=!1;try{const F=await ae.get("students",w);F&&F.students&&(N.students=F.students||[],N.pagination=F.pagination||{page:1,limit:10,total:0,pages:1},ye(),P=!0)}catch(F){console.warn("IDB read students list warning:",F)}P||Ee.skeleton(U,"table");try{const F=await z.get("/api/students",{page:e,limit:10,search:s,status:r});F.success&&F.data&&(N.students=F.data.students||[],N.pagination=F.data.pagination||{page:1,limit:10,total:0,pages:1},await ae.set("students",w,F.data),ye())}catch(F){P||(k.error(F.message||"Failed to load students"),U.innerHTML='<div class="text-center p-5 text-muted">Error loading students list.</div>')}}function ye(){if(N.students.length===0){Ae.emptyState(U,{icon:"\u{1F393}",title:"No Students Found",description:"No student records match your search or status filter. Click below to enroll a new member.",actionText:"+ Add Student",onAction:()=>{const i=K.querySelector("#addStudentBtn");i&&i.click()}});return}let e=N.students.map(i=>{let d="background: rgba(255,255,255,0.08); color: #ccc;";i.status==="active"?d="background: rgba(0, 184, 148, 0.2); color: var(--color-success, #00b894);":i.status==="pending_payment"||i.status==="pending"?d="background: rgba(245, 158, 11, 0.2); color: var(--color-warning, var(--color-warning));":i.status==="expired"?d="background: rgba(214, 48, 49, 0.2); color: var(--color-danger, #d63031);":i.status==="suspended"&&(d="background: rgba(253, 203, 110, 0.2); color: var(--color-warning, #fdcb6e);");const l=i.plan?.name||"-",p=i.seat?.seatNumber||"-",S=i.expiryDate?new Date(i.expiryDate).toLocaleDateString("en-IN"):"-";return`
...
`),h=document.createElement("a");h.href=encodeURI(S),h.download=`selected_students_${Date.now()}.csv`,document.body.appendChild(h),h.click(),document.body.removeChild(h),k.success(`Exported ${i.length} students to CSV`)}),N.pagination.pages>1){const i=document.createElement("div");i.className="d-flex justify-content-between align-items-center p-3 border-top";const d=document.createElement("button");d.className="btn btn-sm btn-outline-secondary",d.textContent="Previous",d.disabled=N.pagination.page<=1,d.onclick=()=>ee(N.pagination.page-1);const l=document.createElement("button");l.className="btn btn-sm btn-outline-secondary",l.textContent="Next",l.disabled=N.pagination.page>=N.pagination.pages,l.onclick=()=>ee(N.pagination.page+1);const p=document.createElement("span");p.className="text-muted small",p.textContent=`Page ${N.pagination.page} of ${N.pagination.pages} (${N.pagination.total} total)`,i.appendChild(d),i.appendChild(p),i.appendChild(l),U.appendChild(i)}}let H=null;async function it(){if(H&&Array.isArray(H.branches)&&H.branches.length>0)return Promise.all([z.get("/api/plans").catch(()=>null),z.get("/api/seats?status=available").catch(()=>null),z.get("/api/custom-fields/all").catch(()=>null),z.get("/api/custom-fields/templates/active").catch(()=>null),z.get("/api/branches/public-list").catch(()=>null)]).then(([e,s,r,w,P])=>{e?.data&&(H.plans=e.data),s?.data&&(H.seats=s.data),r?.data&&(H.customFields=r.data),w?.data&&(H.template=w.data),P?.data&&(H.branches=P.data)}).catch(()=>{}),H;try{const[e,s,r,w,P]=await Promise.all([z.get("/api/plans").catch(()=>({data:[]})),z.get("/api/seats?status=available").catch(()=>({data:[]})),z.get("/api/custom-fields/all").catch(()=>({data:[]})),z.get("/api/custom-fields/templates/active").catch(()=>({data:{}})),z.get("/api/branches/public-list").catch(()=>({data:[]}))]);return H={plans:e?.data||[],seats:s?.data||[],customFields:r?.data||[],template:w?.data||{},branches:P?.data||[]},H}catch(e){return console.error("Error fetching student modal dependencies:",e),{plans:[],seats:[],customFields:[],template:{},branches:[]}}}async function we(e=null){let s=!!e;if(e&&e._id)try{const t=await z.get(`/api/students/${e._id}`);t?.data&&(e=t.data)}catch{}const r=await it(),w=Array.isArray(r.plans)?r.plans:[],P=Array.isArray(r.seats)?r.seats:[],F=Array.isArray(r.customFields)?r.customFields:[],y=r.template||{};let f=Array.isArray(r.branches)&&r.branches.length>0?r.branches:window.store?.branches&&window.store.branches.length>0?window.store.branches:[];if(f.length===0)try{const t=await z.get("/api/branches/public-list");t?.data&&Array.isArray(t.data)&&t.data.length>0&&(f=t.data,H&&(H.branches=f))}catch{}let i='<option value="">-- Select Plan (Optional) --</option>',d='<option value="">-- Select Seat (Optional) --</option>',l=P;w.forEach(t=>{const o=e&&e.plan&&(e.plan._id===t._id||e.plan===t._id)?"selected":"",g=Number(t.price)||0,x=Number(t.discount)||0,L=Math.round(t.effectivePrice!==void 0?t.effectivePrice:g*(1-x/100)),I=x>0?` [${x}% OFF, was \u20B9${g.toLocaleString("en-IN")}]`:"";i+=`<option value="${t._id}" data-price="${g}" data-discount="${x}" data-effective="${L}" ${o}>${a(t.name)} - \u20B9${L.toLocaleString("en-IN")} (${t.duration||1} ${t.durationType||"months"})${I}</option>`}),P.forEach(t=>{const o=e&&e.seat&&(e.seat._id===t._id||e.seat===t._id)?"selected":"";d+=`<option value="${t._id}" ${o}>${a(t.seatNumber)} (${a(t.zone)} - ${a(t.type)})</option>`}),e&&e.seat&&typeof e.seat=="object"&&(d.includes(e.seat._id)||(d+=`<option value="${e.seat._id}" selected>${a(e.seat.seatNumber)} (Current)</option>`));function p(t){if(!e)return"";const o=(t||"").toLowerCase().replace(/[^a-z0-9]/g,"");if(o==="branch"||o==="studycentre"||o==="center"||o==="centre")return e.branch?typeof e.branch=="object"&&e.branch._id?String(e.branch._id):String(e.branch):e.customFields&&(e.customFields.branch||e.customFields.studycentre||e.customFields.center)||"";if(e[t]!==void 0&&e[t]!==null&&e[t]!=="")return e[t];if(o==="targetexams"||o==="target_exams"||o==="competitiveexams"||o==="exams")return e.targetExams||e.customFields?.targetExams||e.customFields?.target_exams||"";if(o==="gender")return e.gender||e.customFields?.gender||e.customFields?.Gender||"";if(o==="bloodgroup"||o==="blood"||o==="blood_group")return e.bloodGroup||e.customFields?.bloodGroup||e.customFields?.blood_group||e.customFields?.bloodgroup||e.customFields?.BloodGroup||"";if(o==="occupation"||o==="collegeorcompany"||o==="college_or_company")return e.occupation||e.collegeOrCompany||e.customFields?.occupation||e.customFields?.collegeOrCompany||e.customFields?.college_or_company||"";if(o==="idprooftype"||o==="id_proof_type"||o==="idtype")return e.idProof?.type||e.customFields?.idProofType||e.customFields?.id_proof_type||e.customFields?.idprooftype||"Aadhaar Card";if(o==="idproofnumber"||o==="id_proof_number"||o==="idnumber"||o==="aadhaar"||o==="pan")return e.idProof?.number||e.customFields?.idProofNumber||e.customFields?.id_proof_number||e.customFields?.idproofnumber||e.customFields?.aadhaar||e.customFields?.pan||"";if(o==="idproofimage"||o==="idproof"||o==="id_proof_image")return e.idProof?.image||e.customFields?.idProofImage||e.customFields?.id_proof_image||e.customFields?.idproofimage||"";if(o==="emergencycontactname"||o==="emergency_contact_name"||o==="parentname"||o==="fathername"||o==="parentguardianname"||o==="guardianname"||o==="parentcontactname")return e.emergencyContact?.name||e.customFields?.parent___guardian_name||e.customFields?.parentguardianname||e.customFields?.emergencyContactName||e.customFields?.parentName||e.customFields?.fatherName||e.customFields?.emergency_contact_name||e.customFields?.emergencycontactname||"";if(o==="emergencycontactphone"||o==="emergencycontact"||o==="parentphone"||o==="emergency_contact_phone"||o==="emergencyphone")return e.emergencyContact?.phone||e.customFields?.emergencyContactPhone||e.customFields?.emergencyContact||e.customFields?.emergencycontact||e.customFields?.parentPhone||e.customFields?.emergency_contact_phone||e.customFields?.emergencycontactphone||"";if(o==="emergencycontactrelation"||o==="emergency_contact_relation"||o==="parentrelation"||o==="relation"||o==="relationship")return e.emergencyContact?.relation||e.customFields?.relationship||e.customFields?.relation||e.customFields?.emergencyContactRelation||e.customFields?.parentRelation||e.customFields?.emergency_contact_relation||e.customFields?.emergencycontactrelation||"Parent";if(o==="dateofbirth"||o==="dob"||o==="date_of_birth"||o==="birthdate"){const g=e.dateOfBirth||e.dob||e.customFields&&(e.customFields.dateOfBirth||e.customFields.dob||e.customFields.dateofbirth||(e.customFields instanceof Map?e.customFields.get("dateOfBirth")||e.customFields.get("dob")||e.customFields.get("dateofbirth"):null));if(g){const x=new Date(g);return isNaN(x.getTime())?"":x.toISOString().split("T")[0]}return""}if(o==="address")return e.address||e.customFields?.address||"";if(o==="city")return e.city||e.customFields?.city||"";if(o==="state")return e.state||e.customFields?.state||"";if(o==="pincode")return e.pincode||e.customFields?.pincode||"";if(e.customFields){if(e.customFields instanceof Map){if(e.customFields.has(t))return e.customFields.get(t)||"";for(const[g,x]of e.customFields.entries())if(g.toLowerCase().replace(/[^a-z0-9]/g,"")===o)return x||""}else if(typeof e.customFields=="object"){if(e.customFields[t]!==void 0&&e.customFields[t]!==null)return e.customFields[t];for(const[g,x]of Object.entries(e.customFields))if(g.toLowerCase().replace(/[^a-z0-9]/g,"")===o)return x||""}}return""}const S=y.sections&&y.sections.length>0?y.sections.filter(t=>!t.isHidden).sort((t,o)=>(t.order||0)-(o.order||0)):[{name:"personal",label:"Personal & Contact Details",icon:"personal"},{name:"academic",label:"Academic Goals & KYC Verification",icon:"academic"},{name:"seat",label:"Declaration & Signature",icon:"seat"}],h=new Set(s?[]:["name","phone","plan","seat","status","paymentmode","payment_mode"]),m=new Set(["plan","seat","status","notes","rfidCardNumber","biometricId","paymentMode","payment_mode"]),A=[];(F||[]).forEach(t=>{if(t.isActive===!1)return;const o=(t.fieldName||"").trim().toLowerCase();o&&(m.has(t.fieldName)||h.has(o)||(h.add(o),A.push(t)))}),A.sort((t,o)=>(t.order||0)-(o.order||0));const T=A.some(t=>{const o=(t.fieldName||"").toLowerCase().replace(/[^a-z0-9]/g,"");return o==="branch"||o==="studycentre"||o==="center"||o==="centre"||t.type==="branch"}),_=new Map,M={personal:"\u{1F464}",academic:"\u{1F3AF}",plan:"\u23F0",payment:"\u{1F4B3}",seat:"\u{1FA91}",contact:"\u{1F4CD}",kyc:"\u{1FAAA}",address:"\u{1F4CD}"};S.forEach(t=>{_.set(t.name,{key:t.name,label:t.label,icon:M[t.icon]||(t.icon&&t.icon.length<=4?t.icon:"")||"\u{1F4DD}",fields:[]})}),A.forEach(t=>{const o=t.section||"personal";if(_.has(o))_.get(o).fields.push(t);else{const g=_.values().next().value;g&&g.fields.push(t)}});function b(t){const o=p(t.fieldName),g=t.required?' <span class="text-danger">*</span>':"",x=t.type==="textarea"||t.type==="address_autocomplete"||t.type==="aadhaar_pan"||t.type==="exam_badge"||t.type==="signature_pad"||t.colSpan===12||t.colSpan===2?"col-12":"col-md-6",L=t.showIf||(t.conditional?.enabled?{field:t.conditional.dependsOn,operator:t.conditional.operator||"equals",value:t.conditional.showWhen}:null),I=L?`data-depends-on="${a(L.field)}" data-show-when="${a(L.value||"")}" data-operator="${a(L.operator||"equals")}" style="display:none;"`:"",R=t.helpText?`<small class="text-muted d-block" style="font-size: 0.72rem; margin-top: 3px;">${a(t.helpText)}</small>`:"";if(t.type==="photo_upload")return`
        ...
        `;if(t.type==="aadhaar_pan"){const D=String(p("idProofType")||"").toLowerCase();return`
          <div class="col-12 mt-2 dynamic-field-wrapper" ${I}>
            ...
            <div class="row g-2 mb-2" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 200px), 1fr)); gap: 10px;">
              <div>
                ...
                <select class="form-select form-control" name="idProof.type">
                  <option value="Aadhaar Card" ${D.includes("aadhaar")||D===""?"selected":""}>Aadhaar Card</option>
                  <option value="PAN Card" ${D.includes("pan")?"selected":""}>PAN Card</option>
                  <option value="Driving License" ${D.includes("driving")?"selected":""}>Driving License</option>
                  <option value="Voter ID" ${D.includes("voter")?"selected":""}>Voter ID</option>
                  <option value="Passport" ${D.includes("passport")?"selected":""}>Passport</option>
                  <option value="Student / College ID" ${D.includes("student")||D.includes("college")?"selected":""}>Student / College ID</option>
                  <option value="Other Govt ID" ${D.includes("other")||D.includes("govt")?"selected":""}>Other Govt ID</option>
                </select>
              </div>
              <div>
        ...
        `;if(t.type==="exam_badge"){const D=t.options&&t.options.length>0?t.options:["UPSC","MPSC","Banking / IBPS","SSC CGL","JEE / NEET","CA / CS","GATE","UGC NET","State PSC","Law / CLAT","Defence / NDA","Other"],B=p(t.fieldName),u=Array.isArray(B)?B:typeof B=="string"?B.split(",").map(G=>G.trim()).filter(Boolean):[];return`
          <div class="col-12 mt-2 dynamic-field-wrapper" ${I}>
        ...
        `}if(t.type==="blood_group"){const D=["A+","A-","B+","B-","O+","O-","AB+","AB-"],B=String(o||"").toUpperCase().trim();return`
          <div class="${x} dynamic-field-wrapper" ${I}>
            ...
            <select class="form-select form-control custom-dyn-input" data-field="${a(t.fieldName)}" name="${a(t.fieldName)}" ${t.required?"required":""}>
              ...
              ${D.map(u=>`<option value="${u}" ${B===u?"selected":""}>${u}</option>`).join("")}
            </select>
            ${R}
          </div>
        `}if(t.type==="star_rating"){const D=parseInt(o,10)||5;return`
          <div class="${x} dynamic-field-wrapper" ${I}>
            ...
            <div class="star-rating-wrap modal-star-rating" data-field="${a(t.fieldName)}" style="display: inline-flex; gap: 6px; font-size: 1.4rem; cursor: pointer;">
              ${[1,2,3,4,5].map(B=>`<span class="star-rating-item ${B<=D?"active":""}" data-val="${B}" style="color: ${B<=D?"var(--color-warning)":"#d1d5db"};">\u2605</span>`).join("")}
            </div>
            <input type="hidden" class="custom-dyn-input" data-field="${a(t.fieldName)}" name="${a(t.fieldName)}" value="${D}">
            ${R}
          </div>
        ...
        `;const X=(t.fieldName||"").toLowerCase().replace(/[^a-z0-9]/g,"");if(X==="branch"||X==="studycentre"||X==="centre"||X==="center"||t.type==="branch"){const D=String(o||"").trim().toLowerCase();let B='<option value="">-- Select Study Centre / Branch --</option>';return(f||[]).forEach(u=>{const G=String(u._id||u.id||""),me=u.name||"Main Campus",Le=u.city?` (${u.city})`:"",Ie=D&&D===G.toLowerCase()||D&&D===me.toLowerCase();B+=`<option value="${a(G)}" ${Ie?"selected":""}>${a(me+Le)}</option>`}),`
          <div class="${x} dynamic-field-wrapper" ${I}>
        ...
        `}if(t.type==="select"){const D=String(o||"").toLowerCase().trim();return`
          <div class="${x} dynamic-field-wrapper" ${I}>
            <label class="form-label" style="font-weight: 500;">${a(t.label)}${g}</label>
            <select class="form-select form-control custom-dyn-input" data-field="${a(t.fieldName)}" name="${a(t.fieldName)}" ${t.required?"required":""}>
              <option value="">-- Select --</option>
              ${(t.options||[]).map(B=>`<option value="${a(B)}" ${D===String(B).toLowerCase().trim()?"selected":""}>${a(B)}</option>`).join("")}
            </select>
            ${R}
          </div>
        `}if(t.type==="radio"){const D=String(o||"").toLowerCase().trim();return`
          <div class="${x} dynamic-field-wrapper" ${I}>
            <label class="form-label" style="font-weight: 500;">${a(t.label)}${g}</label>
            <div style="display: flex; gap: 12px; flex-wrap: wrap; margin-top: 4px;">
              ${(t.options||[]).map(B=>`
                <label style="display: inline-flex; align-items: center; gap: 4px; font-size: 0.88rem; cursor: pointer;">
                  <input type="radio" class="custom-dyn-radio" name="${a(t.fieldName)}" data-field="${a(t.fieldName)}" value="${a(B)}" ${D===String(B).toLowerCase().trim()?"checked":""}>
                  ${a(B)}
                </label>
              `).join("")}
            </div>
        ...
        `}if(t.type==="checkbox"||t.type==="terms_checkbox"||t.type==="consent_checkbox"){const D=o===!0||o==="true"||o==="on"||o===1;return`
          <div class="col-12 mt-1 dynamic-field-wrapper" ${I}>
            <label class="form-check" style="display: inline-flex; align-items: center; gap: 8px; cursor: pointer; margin: 0;">
              <input type="checkbox" class="form-checkbox custom-dyn-checkbox" name="${a(t.fieldName)}" data-field="${a(t.fieldName)}" ${D?"checked":""} ${t.required?"required":""}>
    ...
    `;const M=new ge({title:`\u{1F464} Student Profile: ${e.name}`,content:_,size:"lg"});M.show(),rt(e._id,_),_.querySelectorAll(".btn-copy-text").forEach(b=>{b.addEventListener("click",v=>{v.stopPropagation();const C=b.getAttribute("data-copy");C&&nt(C,b)})}),_.querySelector(".btn-profile-remind")?.addEventListener("click",()=>{Ue(e,e.balanceDue>0?"balance_due":"renewal_reminder")}),_.querySelector(".btn-profile-idcard")?.addEventListener("click",()=>{M.close(),Me(e)}),_.querySelector(".btn-profile-pdfform")?.addEventListener("click",()=>{M.close(),je(e,{business:window.store?.settings?.businessProfile,receiptConfig:window.store?.settings?.receipt})}),_.querySelector(".btn-profile-edit")?.addEventListener("click",()=>{M.close(),we(e)})}async function st(e=[],s){let r=e,w=[],P=[];try{const[A,T]=await Promise.all([z.get("/api/shifts?limit=50"),z.get("/api/plans?limit=50")]);w=A?.data?.shifts||A?.data||[],P=T?.data?.plans||T?.data||[]}catch{}const F=w.map(A=>`<option value="${a(A._id)}">${a(A.name)}</option>`).join(""),y=P.map(A=>`<option value="${a(A._id)}">${a(A.name)}</option>`).join(""),f=`Hi {name}! \u{1F44B}
...
\u2014 {library}`,i=`
      ...
      `});if(!d)return;let l="selected",p=[...e];const S=d.querySelector("#wa-count-num"),h=d.querySelector("#wa-blast-send-btn");function m(){S&&(S.textContent=p.length),h&&(h.textContent=`\u{1F4F2} Open WA Links (${p.length})`)}d.querySelectorAll(".wa-filter-btn").forEach(A=>{A.onclick=()=>{d.querySelectorAll(".wa-filter-btn").forEach(b=>b.classList.replace("btn-primary","btn-outline-secondary")),A.classList.replace("btn-outline-secondary","btn-primary"),l=A.dataset.filter;const T=d.querySelector("#wa-filter-sub"),_=d.querySelector("#wa-shift-select"),M=d.querySelector("#wa-plan-select");l==="by_shift"?(T&&(T.style.display="block"),_&&(_.style.display="block"),M&&(M.style.display="none")):l==="by_plan"?(T&&(T.style.display="block"),_&&(_.style.display="none"),M&&(M.style.display="block")):T&&(T.style.display="none"),l==="selected"&&(p=[...e],m())}}),d.querySelector("#wa-blast-load-btn")?.addEventListener("click",async()=>{try{let A={status:"active",limit:500};if(l==="by_shift"){const _=d.querySelector("#wa-shift-id")?.value;if(!_){k.error("Please select a shift");return}A.shift=_}else if(l==="by_plan"){const _=d.querySelector("#wa-plan-id")?.value;if(!_){k.error("Please select a plan");return}A.plan=_}else if(l==="selected"){p=[...e],m();return}const T=await z.get("/api/students",A);p=T?.data?.students||T?.data||[],m(),k.success(`${p.length} recipients loaded`)}catch{k.error("Failed to load recipients")}}),d.querySelector("#wa-blast-send-btn")?.addEventListener("click",async()=>{if(p.length===0){k.error("No recipients");return}const A=d.querySelector("#wa-blast-message")?.value||f;let T="Study Library";try{T=(await z.get("/api/settings"))?.data?.businessProfile?.businessName||T}catch{}ge.closeAll();let _=0;for(const M of p){const b=(M.phone||"").replace(/[^0-9]/g,"");if(!b||b.length<10)continue;const v=b.length===10?"91"+b:b,C=M.expiryDate?new Date(M.expiryDate).toLocaleDateString("en-IN"):"N/A",j=`https://wa.me/${v}`,$=A.replace(/{name}/g,M.name||"Student").replace(/{expiry}/g,C).replace(/{plan}/g,M.plan?.name||"your plan").replace(/{seat}/g,M.seat?.seatNumber||"N/A").replace(/{link}/g,j).replace(/{library}/g,T),V=`https://wa.me/${v}?text=${encodeURIComponent($)}`;window.open(V,"_blank"),_++,_<p.length&&await new Promise(q=>setTimeout(q,700))}k.success(`\u{1F4F2} Opened ${_} WhatsApp link(s). Check your browser tabs!`)})}async function Ue(e,s="renewal_reminder"){try{Ee.show("Preparing WhatsApp reminder & UPI payment link...");let r="";try{const w=await z.post("/api/messages/send-reminder",{studentId:e._id,reminderType:s});w.success&&w.data&&(r=w.data.whatsappUrl||w.data.waUrl)}catch(w){console.warn("Backend send-reminder error, generating direct WhatsApp payment link:",w)}if(!r){const w=e.balanceDue||e.pendingFine||e.plan?.price||0,P=s==="balance_due"?"Balance Due":"Membership Renewal",F=e.expiryDate?new Date(e.expiryDate).toLocaleDateString("en-IN"):"";r=wt.generateWhatsAppPaymentLink({phone:e.phone,studentName:e.name,amount:w,dueDate:F,planName:e.plan?.name||"",paymentType:P})}Ee.hide(),r?(/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)?window.location.href=r:window.open(r,"_blank")||(window.location.href=r),k.success(`WhatsApp reminder opened for ${a(e.name||"Student")}!`)):k.error("Could not construct WhatsApp link")}catch(r){Ee.hide(),k.error(r.message||"Failed to trigger reminder")}}async function rt(e,s){const r=s.querySelector("#student-analytics-content"),w=s.querySelector("#student-analytics-badges");if(!r)return;function P(y){if(!y)return;if(w){const i=y.consistencyScore||0,d=y.currentStreak||0,l=xt(i,100,d);w.innerHTML=`
          ${l}
            ...
            ">
              ...
              <div style="display: flex; flex-direction: column; align-items: center; padding: 8px 10px 4px 10px; text-align: center;">
                <div style="width: 68px; height: 68px; border-radius: 12px; background: #eef2ff; border: 2.5px solid ${b}; overflow: hidden; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; font-weight: 800; color: ${b}; margin-bottom: 4px; box-shadow: 0 3px 10px rgba(0,0,0,0.1);">
                  ${e.photo?`<img src="${e.photo}" style="width: 100%; height: 100%; object-fit: cover;">`:A}
                </div>
                ...
                <div style="display: flex; gap: 4px; align-items: center; justify-content: center; flex-wrap: wrap;">
                  ...
                  ${j&&m?`<span style="background: rgba(220,38,38,0.12); color: #dc2626; font-size: 0.65rem; font-weight: 800; padding: 1px 5px; border-radius: 4px;">\u{1FA78} ${a(m)}</span>`:""}
                </div>
              </div>
              ...
              <div style="padding: 10px 12px; display: flex; gap: 12px; align-items: center; flex: 1;">
                ...
                <div style="flex: 1; min-width: 0;">
                  ...
                  <div style="display: flex; gap: 4px; align-items: center; margin-bottom: 4px; flex-wrap: wrap;">
                    ...
                    ${j&&m?`<span style="background: rgba(220,38,38,0.12); color: #dc2626; font-size: 0.62rem; font-weight: 800; padding: 1px 5px; border-radius: 3px;">\u{1FA78} ${a(m)}</span>`:""}
                  </div>
      ...
      `,q.querySelector(".btn-opt-horiz")?.addEventListener("click",()=>{_="horizontal",W()}),q.querySelector(".btn-opt-vert")?.addEventListener("click",()=>{_="vertical",W()}),q.querySelector(".btn-side-front")?.addEventListener("click",()=>{M="front",W()}),q.querySelector(".btn-side-back")?.addEventListener("click",()=>{M="back",W()}),q.querySelector(".btn-side-dual")?.addEventListener("click",()=>{M="dual",W()}),q.querySelector("#id-studio-color")?.addEventListener("input",E=>{b=E.target.value,W()}),q.querySelector("#id-studio-theme-select")?.addEventListener("change",E=>{v=E.target.value,W()}),q.querySelector("#toggle-id-qr")?.addEventListener("change",E=>{C=E.target.checked,W()}),q.querySelector("#toggle-id-blood")?.addEventListener("change",E=>{j=E.target.checked,W()}),q.querySelector("#toggle-id-emergency")?.addEventListener("change",E=>{$=E.target.checked,W()}),q.querySelector("#toggle-id-stamp")?.addEventListener("change",E=>{V=E.target.checked,W()}),q.querySelector("#btn-close-id-studio")?.addEventListener("click",()=>{$e.close()}),q.querySelector("#btn-print-id-card")?.addEventListener("click",()=>{M!=="dual"&&(M="dual",W()),setTimeout(()=>{window.print()},300)});const ve=async(E,c)=>{try{window.html2canvas||await new Promise((ie,se)=>{const re=document.createElement("script");re.src="https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js",re.onload=ie,re.onerror=se,document.head.appendChild(re)});const oe=q.querySelector(E);if(!oe){k.warning("Please switch to the selected side first.");return}const Se=await window.html2canvas(oe,{scale:3,useCORS:!0,backgroundColor:null}),de=document.createElement("a");de.download=c,de.href=Se.toDataURL("image/png"),de.click(),k.success("ID Card downloaded successfully!")}catch(oe){k.error("PNG download error: "+oe.message)}};q.querySelector("#btn-download-front-png")?.addEventListener("click",()=>{const E=(e.studentId||e.name||"student").replace(/\s+/g,"_");ve(".id-card-front",`ID_Front_${E}.png`)}),q.querySelector("#btn-download-back-png")?.addEventListener("click",()=>{const E=(e.studentId||e.name||"student").replace(/\s+/g,"_");ve(".id-card-back",`ID_Back_${E}.png`)}),q.querySelector("#btn-download-dual-png")?.addEventListener("click",()=>{const E=(e.studentId||e.name||"student").replace(/\s+/g,"_");ve("#id-card-render-stage",`ID_Dual_${E}.png`)})};W();const $e=new ge({title:`\u{1FAAA} Student ID Card Studio: ${a(e.name)}`,content:q,size:"xl"});$e.show()}function He(e){const s=(e.phone||"").replace(/[^0-9]/g,"").slice(-10),r=e.dateOfBirth?new Date(e.dateOfBirth).toISOString().slice(0,10).replace(/-/g,""):"",w=Math.floor(1e5+Math.random()*9e5).toString(),P=`
    ...
    `,F=async f=>{const i=document.getElementById("reset-pwd-input")?.value?.trim(),d=document.getElementById("reset-toggle-email")?.checked,l=document.getElementById("reset-toggle-wa")?.checked;if(!i||i.length<4)return k.error("Password must be at least 4 characters long"),!1;try{const p=await z.post(`/api/students/${e._id}/reset-password`,{newPassword:i,sendEmail:d});return p.success?(k.success(`Password updated for ${e.name}!`),l&&p.data?.whatsappUrl&&window.open(p.data.whatsappUrl,"_blank"),!0):(k.error(p.message||"Failed to update password"),!1)}catch(p){return k.error(p.message||"Password update failed"),!1}},y=ge.show({title:"\u{1F511} Reset Student Portal Password",content:P,confirmText:"\u{1F4BE} Save & Update Password",confirmClass:"btn-primary",onConfirm:async f=>await F(f)});setTimeout(()=>{const f=document.getElementById("reset-pwd-input");document.getElementById("preset-random-pin")?.addEventListener("click",()=>{f&&(f.value=Math.floor(1e5+Math.random()*9e5).toString())}),document.getElementById("preset-phone-pin")?.addEventListener("click",()=>{f&&s&&(f.value=s)}),document.getElementById("preset-dob-pin")?.addEventListener("click",()=>{f&&r&&(f.value=r)}),document.getElementById("btn-toggle-reset-eye")?.addEventListener("click",()=>{f&&(f.type=f.type==="password"?"text":"password")})},100)}const We=K.querySelector("#studentSearch");We&&We.addEventListener("input",ft(()=>ee(1),250));const Ve=K.querySelector("#studentStatusFilter");return Ve&&Ve.addEventListener("change",()=>ee(1)),setTimeout(()=>{fe(),ee(1)},0),typeof window<"u"&&window.FAB&&window.FAB.mount({icon:"\u{1F393}",label:"Student Actions",color:"var(--color-primary, #6c5ce7)",actions:[{icon:"\u2795",label:"Add Student",onClick:()=>{we()}},{icon:"\u23F3",label:"Waiting List",onClick:()=>{window.location.hash="#/operations"}},{icon:"\u{1F4E4}",label:"Export Students",onClick:async()=>{const e=N.students||[];if(e.length===0){k.info("No students loaded to export");return}const s=["Student ID","Full Name","Phone","Email","Plan","Seat","Blood Group","Gender","Status","Expiry Date"],r=e.map(f=>[f.studentId||"",f.name||"",f.phone||"",f.email||"",f.plan?.name||"",f.seat?.seatNumber||"",f.bloodGroup||"",f.gender||"",f.status||"",f.expiryDate?new Date(f.expiryDate).toLocaleDateString("en-IN"):""]);try{await Ut.downloadCSV({headers:s,rows:r,filename:`students_export_${new Date().toISOString().split("T")[0]}.csv`});k.success(`Exported ${e.length} student(s) to CSV`)}catch(err){k.error("Export failed: "+err.message)}}},{icon:"\u{1F50D}",label:"Search Student",onClick:()=>{const e=K.querySelector("#studentSearch")||document.querySelector("#studentSearch");e&&(e.focus(),e.scrollIntoView({behavior:"smooth"}))}}]}),K}export{Ft as render};
