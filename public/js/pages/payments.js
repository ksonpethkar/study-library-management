import"../app.js";import"../i18n.js";import{Toast as c,Modal as U,Loading as P,Confirm as G,escapeHTML as b,copyToClipboard as Y,UI as H}from"../ui.js";import{SmartFormatters as q}from"../utils/smartFormatters.js";import k from"../api.js";import{IDBStorage as A}from"../utils/idbStorage.js";import{OptimisticUI as te}from"../utils/optimisticUI.js";import{buildReceiptHTML as de,printReceiptDocument as ce}from"../pdfGenerator.js";import{PaymentStudio as B}from"../paymentStudio.js";import{initSwipeCards as ae}from"../utils/mobileGestures.js";const N=p=>q.currency(p),j=p=>p?new Date(p).toLocaleDateString("en-IN"):"-";async function pe(p){p||(p=document.createElement("div"),p.className="page-container"),p.innerHTML=`
...
`});const n=new Blob([r],{type:"text/csv;charset=utf-8;"}),s=URL.createObjectURL(n),l=document.createElement("a");l.href=s,l.setAttribute("download",`Payments_Ledger_${new Date().toISOString().split("T")[0]}.csv`),document.body.appendChild(l),l.click(),document.body.removeChild(l),c.success("Payments CSV downloaded successfully!")}catch(a){P.hide(),c.error("Failed to export CSV: "+a.message)}}async function R(){try{const a=await A.get("payments","stats");if(a){const i=document.getElementById("statToday"),o=document.getElementById("statMonth"),e=document.getElementById("statPending");i&&(i.textContent=N(a.todayRevenue)),o&&(o.textContent=N(a.monthRevenue)),e&&(e.textContent=N(a.totalPending))}}catch(a){console.warn("IDB read payments stats warning:",a)}try{const a=await k.get("/api/payments/stats");if(a.success&&a.data){await A.set("payments","stats",a.data);const i=document.getElementById("statToday"),o=document.getElementById("statMonth"),e=document.getElementById("statPending");i&&(i.textContent=N(a.data.todayRevenue)),o&&(o.textContent=N(a.data.monthRevenue)),e&&(e.textContent=N(a.data.totalPending))}}catch(a){console.error("Error loading stats",a)}}async function J(a,i,o){if(await G.show({title:"Verify UPI Payment",message:`Confirm that UTR / Reference "${i||"N/A"}" (Invoice ${o||a}) matches your bank statement and mark as Paid?`,confirmText:"\u2705 Yes, Verify & Activate Student",cancelText:"Cancel"}))try{const e=await k.put(`/api/payments/${a}/verify-utr`);e.success?(c.success(e.message||"Payment verified successfully!"),await A.clear("payments"),L(),R()):c.error(e.message||"Failed to verify payment")}catch(e){c.error(e.message||"Verification failed")}}async function K(a,i){const o=prompt(`Enter rejection reason for payment ${i||""}:`,"UTR not found in bank statement / Invalid amount");if(!(!o||!o.trim()))try{const e=await k.put(`/api/payments/${a}/reject-utr`,{reason:o.trim()});e.success?(c.warning(e.message||"Payment marked as rejected"),await A.clear("payments"),L(),R()):c.error(e.message||"Failed to reject payment")}catch(e){c.error(e.message||"Rejection failed")}}function W(a,i){if(!i)return;if(!a||a.length===0){H.emptyState(i,{icon:"\u{1F4B3}",title:"No Payments Found",description:"No fee collection transactions recorded matching your filter. Click below to collect fee.",actionText:"+ Collect Fee Payment",onAction:()=>{const e=document.getElementById("btnCollectPayment");e&&e.click()}});return}i.innerHTML=a.map(e=>`
        ...
        `;const l=new U({title:"Collect Fee Payment",content:s,size:"md"});l.show();const d=s.querySelector("#paymentForm"),m=s.querySelector("#planSelect"),y=s.querySelector("#payAmount"),F=s.querySelector("#payDiscount"),E=s.querySelector("#payLateFee"),h=s.querySelector("#finalAmountDisplay"),g=s.querySelector("#studentSelect"),C=s.querySelector("#payMethodSelect"),x=s.querySelector("#payMethodContext"),v=s.querySelector("#payTxnLabel"),I=s.querySelector("#payTransactionId"),$=s.querySelector("#utrWarnMsg"),ee=()=>{const u=C?.value||"upi",w=parseFloat(y?.value)||0,T=parseFloat(F?.value)||0,S=parseFloat(E?.value)||0,D=Math.max(0,w-T+S);u==="cash"||u==="desk"?(v&&(v.innerHTML="\u{1F4B5} Cash / Front Desk Note (Optional)"),I&&(I.placeholder="e.g. Received at reception desk"),x&&(x.innerHTML=`
                    ...
                    `),$&&($.style.display="none")):u==="bank_transfer"||u==="netbanking"?(v&&(v.innerHTML="\u{1F3DB}\uFE0F Bank NEFT / IMPS / RTGS UTR Number *"),I&&(I.placeholder="e.g. Bank Ref # / IMPS Transaction Reference"),x&&(x.innerHTML=B.renderBankDetailsWidget(),B.attachEventListeners(x)),$&&($.style.display="none")):u==="card"?(v&&(v.innerHTML="\u{1F4B3} POS Slip Code / Card Last 4 Digits"),I&&(I.placeholder="e.g. POS Auth Code #8492 or Card Ending 4321"),x&&(x.innerHTML=`
                    ...
                    `),$&&($.style.display="none")):(v&&(v.innerHTML="\u26A1 UPI / 12-Digit UTR Transaction ID *"),I&&(I.placeholder="e.g. 12-digit UTR (e.g. 423456789012)"),x&&(x.innerHTML=B.renderUPIWidget({amount:D,note:"Library Fee Payment",showUtrInput:!1,mountId:"collect-pay-upi-qr-mount"}),B.attachEventListeners(x)))};C&&(C.addEventListener("change",ee),ee());const V=()=>{const u=parseFloat(y.value)||0,w=parseFloat(F.value)||0,T=parseFloat(E.value)||0,S=Math.max(0,u-w+T);h.textContent=N(S),C?.value==="upi"&&x&&(x.innerHTML=B.renderUPIWidget({amount:S,note:"Library Fee Payment",showUtrInput:!1,mountId:"collect-pay-upi-qr-mount"}),B.attachEventListeners(x))};if([y,F,E].forEach(u=>{u.addEventListener("input",V)}),m.addEventListener("change",()=>{const u=m.options[m.selectedIndex];u&&u.dataset.price&&(y.value=u.dataset.price,F.value=u.dataset.discountAmt||0,V())}),n&&n.plan){const u=n.plan._id||n.plan;m.value=u;const w=m.options[m.selectedIndex];w&&w.dataset.price&&(y.value=w.dataset.price,F.value=w.dataset.discountAmt||0,V())}const re=async()=>{if((C?.value||"upi")!=="upi")return $&&($.style.display="none"),!0;const u=I?.value?.trim()||"";if(!u||!$)return $&&($.style.display="none"),!0;/^\d+$/.test(u)&&u.length!==12?($.textContent=`\u2139\uFE0F UPI UTR is usually 12 digits (currently ${u.length} digits)`,$.style.color="var(--color-warning, #f59e0b)",$.style.display="block"):$.style.display="none";try{const w=await A.get("payments","list__")||[];if(Array.isArray(w)&&w.some(T=>T.transactionId&&T.transactionId.trim().toLowerCase()===u.toLowerCase()))return $.textContent=`\u26A0\uFE0F Warning: Transaction Ref ID "${u}" has already been recorded!`,$.style.color="var(--color-danger, #d63031)",$.style.display="block",!1}catch{}return!0};I&&["input","blur","change"].forEach(u=>I.addEventListener(u,re)),d.addEventListener("submit",async u=>{u.preventDefault();const w=d.querySelector('button[type="submit"]'),T=new FormData(d),S=Object.fromEntries(T.entries());if(!S.student){c.error("Please select a student");return}if(S.plan||delete S.plan,S.amount=parseFloat(S.amount)||0,S.discount=parseFloat(S.discount)||0,S.lateFee=parseFloat(S.lateFee)||0,S.amount<=0){c.error("Payment amount must be greater than 0");return}if(S.discount>S.amount){c.error("Discount amount cannot exceed the payment amount");return}w&&H.buttonLoading(w,!0,"Processing...");try{const D=await k.post("/api/payments",S);D.success?(l.hide(),c.success("Payment collected successfully!"),typeof window.confettiCelebrate=="function"&&window.confettiCelebrate({duration:1800,colors:["#00b894","#00cec9","#55efc4","#6c5ce7","#fdcb6e"]}),typeof window.refreshNotifications=="function"&&window.refreshNotifications(),await A.clear("payments"),R(),L(),Z(),D.data?._id&&z(D.data._id)):c.error(D.message||"Failed to collect payment")}catch(D){c.error(D.message||"An error occurred while saving payment")}finally{w&&H.buttonLoading(w,!1)}})}async function se(a,i){const o=document.createElement("div");o.innerHTML=`
