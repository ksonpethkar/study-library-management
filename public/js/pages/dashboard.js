import{App as Ie}from"../app.js";import{t as R}from"../i18n.js";import{escapeHTML as A,Toast as S,Modal as U}from"../ui.js";import f from"../api.js";import{ChartEngine as H,loadChartJS as Te}from"../charts.js";Te().catch(()=>{});const V=[{id:"kpi_active_students",label:"Active Students",isEnabled:!0,order:1,category:"kpi",icon:"\u{1F465}"},{id:"kpi_available_seats",label:"Available Seats & Live Occupancy",isEnabled:!0,order:2,category:"kpi",icon:"\u{1F4BA}"},{id:"kpi_today_revenue",label:"Today's Fee Collection",isEnabled:!0,order:3,category:"kpi",icon:"\u{1F4B0}"},{id:"kpi_expiring_soon",label:"Expiring in 48 Hours",isEnabled:!0,order:4,category:"kpi",icon:"\u23F0"},{id:"kpi_defaulter_dues",label:"Overdue Fee Balances",isEnabled:!0,order:5,category:"kpi",icon:"\u26A0\uFE0F"},{id:"kpi_total_seats",label:"Total Seat Capacity",isEnabled:!0,order:6,category:"kpi",icon:"\u{1F3E2}"},{id:"kpi_renewals_week",label:"Renewals Due This Week",isEnabled:!0,order:7,category:"kpi",icon:"\u{1F4C5}"},{id:"kpi_occupancy_gauge",label:"Live Seat Occupancy Gauge",isEnabled:!0,order:8,category:"kpi",icon:"\u{1F3AF}"},{id:"kpi_behavior_alerts",label:"At-Risk Student Alerts",isEnabled:!0,order:9,category:"kpi",icon:"\u{1F534}"},{id:"chart_revenue_trend",label:"Monthly Revenue Trend Chart",isEnabled:!0,order:10,category:"chart",icon:"\u{1F4C8}"},{id:"chart_shift_occupancy",label:"Shift Occupancy Distribution Chart",isEnabled:!0,order:11,category:"chart",icon:"\u{1F552}"},{id:"chart_exam_stats",label:"Student Exam Preparation Breakdown",isEnabled:!0,order:12,category:"chart",icon:"\u{1F3AF}"},{id:"quick_actions",label:"Quick 1-Tap Action Toolbar",isEnabled:!0,order:13,category:"action",icon:"\u26A1"},{id:"system_health",label:"System Health Monitor",isEnabled:!0,order:14,category:"kpi",icon:"\u26A1"}],L=c=>new Intl.NumberFormat("en-IN",{style:"currency",currency:"INR",maximumFractionDigits:0}).format(c||0);let q=[];async function De(){try{const c=await f.get("/api/settings/dashboard-widgets");if(c&&c.success&&Array.isArray(c.data)&&c.data.length>0)return c.data.map((i,l)=>{const p=V.find(v=>v.id===i.id)||{};return{id:i.id,label:i.label||p.label||i.id,isEnabled:i.isEnabled!==void 0?!!i.isEnabled:!0,order:i.order!==void 0?Number(i.order):l+1,category:i.category||p.category||"kpi",icon:p.icon||"\u{1F4CA}"}}).sort((i,l)=>i.order-l.order)}catch(c){console.warn("Could not load custom dashboard widgets, using defaults:",c)}return JSON.parse(JSON.stringify(V))}function Le(c){switch(c.id){case"kpi_active_students":return`
        <div class="stat-card card" data-widget-id="kpi_active_students" style="border-left: 4px solid var(--color-primary, #6c5ce7);">
          <div class="stat-card-body">
            ...
            <div class="stat-card-icon" style="background: var(--color-primary-bg, rgba(108, 92, 231, 0.15)); color: var(--color-primary, #6c5ce7);">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="32" height="32"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
            </div>
          </div>
        </div>
        ...
        <div class="stat-card card" data-widget-id="kpi_available_seats" style="border-left: 4px solid var(--color-success, #00b894);">
          <div class="stat-card-body">
            ...
            <div class="stat-card-icon" style="background: rgba(0, 184, 148, 0.15); color: var(--color-success, #00b894);">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="32" height="32"><path d="M5 16V4a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v12"></path><path d="M3 16h18"></path><path d="M5 16v4"></path><path d="M19 16v4"></path></svg>
            </div>
          </div>
        </div>
        ...
        <div class="stat-card card" data-widget-id="kpi_today_revenue" style="border-left: 4px solid var(--color-info, #0984e3);">
          <div class="stat-card-body">
            ...
            <div class="stat-card-icon" style="background: rgba(9, 132, 227, 0.15); color: var(--color-info, #0984e3);">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="32" height="32"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"></rect><line x1="1" y1="10" x2="23" y2="10"></line></svg>
            </div>
          </div>
        </div>
        ...
        <div class="stat-card card" data-widget-id="kpi_expiring_soon" style="border-left: 4px solid #fdcb6e;">
          <div class="stat-card-body">
            ...
            <div class="stat-card-icon" style="background: rgba(253, 203, 110, 0.15); color: #fdcb6e;">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="32" height="32"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
            </div>
          </div>
        </div>
        ...
        <div class="stat-card card" data-widget-id="kpi_total_seats" style="border-left: 4px solid #a29bfe;">
          <div class="stat-card-body">
            ...
            <div class="stat-card-icon" style="background: rgba(162, 155, 254, 0.15); color: #a29bfe;">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="32" height="32"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg>
            </div>
          </div>
        </div>
      ...
      `;default:return""}}function Me(c){const i=c.filter(h=>h.isEnabled).sort((h,m)=>h.order-m.order);if(i.length===0)return`
    ...
    `;let l='<div class="dashboard-dynamic-container">',p=null,v=[];const y=()=>{v.length!==0&&(p==="kpi"?l+=`<div class="stats-grid mb-4" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 230px), 1fr)); gap: 1rem;">${v.join("")}</div>`:p==="chart"?l+=`<div class="charts-grid mb-4" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 340px), 1fr)); gap: 1.5rem;">${v.join("")}</div>`:l+=v.join(""),v=[],p=null)};return i.forEach(h=>{const m=Le(h);m&&(h.category==="kpi"?(p!=="kpi"&&y(),p="kpi",v.push(m)):h.category==="chart"?(p!=="chart"&&y(),p="chart",v.push(m)):(y(),v.push(m),y()))}),y(),l+="</div>",l}function ze(c){let i=JSON.parse(JSON.stringify(q));const l=document.createElement("div");l.className="dashboard-customize-container";const p=()=>{l.innerHTML=`
      ...
      <div id="customize-widget-list" class="d-flex flex-column gap-2" style="max-height: 440px; overflow-y: auto; padding-right: 4px;">
        ${i.map((e,a)=>{let o="badge-primary",b="KPI";return e.category==="chart"?(o="badge-info",b="Chart"):e.category==="action"&&(o="badge-warning",b="Action"),`
              ...
              <div class="d-flex align-items-center gap-3" style="flex: 1; min-width: 0;">
                ...
                <div style="min-width: 0;">
                  <div style="font-weight: 600; font-size: 14px; display: flex; align-items: center; gap: 8px;">
                    <span>${A(e.label)}</span>
                    <span class="badge ${o}" style="font-size: 10px; padding: 2px 6px;">${b}</span>
                  </div>
    ...
    `;const v=l.querySelector("#customize-widget-list");let y=null;v.querySelectorAll(".customize-item").forEach(e=>{e.addEventListener("dragstart",a=>{y=Number(e.getAttribute("data-index")),a.dataTransfer.effectAllowed="move",e.style.opacity="0.4"}),e.addEventListener("dragover",a=>{a.preventDefault(),a.dataTransfer.dropEffect="move",e.style.borderTop="2px solid var(--color-primary, #6c5ce7)"}),e.addEventListener("dragleave",()=>{e.style.borderTop="1px solid var(--color-border, rgba(255,255,255,0.08))"}),e.addEventListener("drop",a=>{a.preventDefault(),e.style.borderTop="1px solid var(--color-border, rgba(255,255,255,0.08))";const o=Number(e.getAttribute("data-index"));if(y!==null&&y!==o){const b=i.splice(y,1)[0];i.splice(o,0,b),i.forEach((k,$)=>{k.order=$+1}),p()}}),e.addEventListener("dragend",()=>{e.style.opacity="1"})}),l.querySelectorAll(".btn-move-up").forEach(e=>{e.onclick=()=>{const a=Number(e.getAttribute("data-index"));if(a>0){const o=i[a];i[a]=i[a-1],i[a-1]=o,i.forEach((b,k)=>{b.order=k+1}),p()}}}),l.querySelectorAll(".btn-move-down").forEach(e=>{e.onclick=()=>{const a=Number(e.getAttribute("data-index"));if(a<i.length-1){const o=i[a];i[a]=i[a+1],i[a+1]=o,i.forEach((b,k)=>{b.order=k+1}),p()}}}),l.querySelectorAll(".widget-toggle-input").forEach(e=>{e.onchange=()=>{const a=Number(e.getAttribute("data-index"));i[a].isEnabled=e.checked,p()}});const h=l.querySelector("#btn-reset-widgets");h&&(h.onclick=()=>{i=JSON.parse(JSON.stringify(V)),p(),S.info("Layout reset to default order")});const m=l.querySelector("#btn-cancel-widgets");m&&(m.onclick=()=>{U.close()});const w=l.querySelector("#btn-save-widgets");w&&(w.onclick=async()=>{w.disabled=!0,w.innerHTML="Saving...";try{i.forEach((a,o)=>{a.order=o+1});const e=await f.put("/api/settings/dashboard-widgets",{widgets:i});e&&e.success?(S.success("Dashboard layout saved successfully!"),q=i,U.close(),c&&c(i)):(S.error(e?.message||"Failed to save widget layout"),w.disabled=!1,w.innerHTML="\u{1F4BE} Save Layout")}catch(e){console.error(e),S.error(e.message||"Error saving dashboard widgets"),w.disabled=!1,w.innerHTML="\u{1F4BE} Save Layout"}})};p(),U.show({title:"\u2699\uFE0F Customize Dashboard Layout & KPI Studio",content:l,size:"lg"})}async function W(c){const i=c||document.getElementById("page-content");if(!i)return;const l=Ie.getUser()||{name:"Admin"},p=["owner","branch_manager","admin"].includes(l.role);q=await De();const v=new Date().getHours(),y=v<12?"\u{1F305} Good Morning":v<17?"\u{1F324}\uFE0F Good Afternoon":"\u{1F319} Good Evening";i.innerHTML=`
  ...
  `;const h=i.querySelectorAll(".portal-tab-pill[data-admin-tab]"),m={"live-ops":i.querySelector("#pane-admin-live-ops"),expiring:i.querySelector("#pane-admin-expiring"),pulse:i.querySelector("#pane-admin-pulse")},w=e=>{h.forEach(a=>{const o=a.getAttribute("data-admin-tab")===e;a.classList.toggle("active",o),a.style.background=o?"var(--color-surface)":"transparent",a.style.color=o?"var(--color-primary)":"var(--color-text-secondary)",a.style.boxShadow=o?"0 2px 8px rgba(0, 0, 0, 0.12)":"none"}),Object.keys(m).forEach(a=>{m[a]&&(m[a].style.display=a===e?"block":"none")})};h.forEach(e=>{e.addEventListener("click",()=>{w(e.getAttribute("data-admin-tab"))})}),(()=>{const e=document.getElementById("btn-refresh-dashboard");e&&(e.onclick=()=>{S.info("Refreshing live dashboard telemetry..."),W(c)});const a=document.getElementById("btn-customize-dashboard");a&&(a.onclick=()=>ze(b=>{W(c)}));const o=document.getElementById("btn-empty-customize");o&&(o.onclick=()=>ze(b=>{W(c)}))})();try{const[e,a,o,b,k,$,G,I]=await Promise.allSettled([f.get("/api/students/stats"),f.get("/api/seats/stats"),f.get("/api/payments/stats"),f.get("/api/attendance/today"),f.get("/api/reports/expiries?days=7"),f.get("/api/shifts/stats"),f.get("/api/reports/revenue"),f.get("/api/students?limit=200")]),F=e.status==="fulfilled"&&e.value?.success?e.value.data:{},M=a.status==="fulfilled"&&a.value?.success?a.value.data:{},T=o.status==="fulfilled"&&o.value?.success?o.value.data:{},N=b.status==="fulfilled"&&b.value?.success?b.value.data:{},j=k.status==="fulfilled"&&k.value?.data?k.value.data:{},D=Array.isArray(j?.students)?j.students:Array.isArray(j)?j:[],P=$.status==="fulfilled"&&$.value?.data?$.value.data:{},z=G.status==="fulfilled"&&G.value?.data?G.value.data:{},J=I.status==="fulfilled"&&I.value?.data?.students?I.value.data.students:Array.isArray(I.value?.data)?I.value.data:[],Y=document.getElementById("dash-hero-occ"),X=document.getElementById("dash-hero-checkin"),Z=document.getElementById("dash-hero-revenue"),ee=document.getElementById("dash-hero-students"),te=document.getElementById("dash-hero-progress-bar"),B=M.occupied??0,C=M.total??0,ae=M.available??Math.max(0,C-B),_=C>0?Math.round(B/C*100):0,Be=N.stats?.totalPresent||N.stats?.totalCheckedIn||0;Y&&(Y.textContent=`${B} / ${C} Seats (${_}%)`),X&&(X.textContent=`\u{1F7E2} ${Be} students currently studying inside`),Z&&(Z.textContent=`${L(T.todayRevenue)} Today`),ee&&(ee.textContent=`${F.active||0} Active Members`),te&&(te.style.width=`${Math.min(100,Math.max(0,_))}%`);const ie=document.getElementById("dash-kpi-active-students"),se=document.getElementById("dash-kpi-total-students-sub");ie&&(ie.textContent=F.active??0),se&&(se.textContent=`Total registered: ${F.total??0}`);const re=document.getElementById("dash-kpi-available-seats"),ne=document.getElementById("dash-kpi-seats-sub");re&&(re.textContent=`${ae} Available`),ne&&(ne.textContent=`${B} / ${C} Occupied (${_}%)`);const de=document.getElementById("dash-kpi-today-revenue"),oe=document.getElementById("dash-kpi-month-rev-sub");de&&(de.textContent=L(T.todayRevenue)),oe&&(oe.textContent=`Month: ${L(T.monthRevenue)}`);const le=document.getElementById("dash-kpi-expiring-soon"),ce=document.getElementById("dash-kpi-expiring-sub"),pe=D.filter(t=>t.daysRemaining!==void 0?t.daysRemaining<=2:t.expiryDate?(new Date(t.expiryDate)-new Date)/864e5<=2:!1).length;le&&(le.textContent=`${pe} Students`),ce&&(ce.textContent=`Next 7 days: ${D.length} total`);const ue=document.getElementById("dash-kpi-defaulter-dues"),ge=document.getElementById("dash-kpi-dues-sub"),_e=T.totalPending||z.summary?.pendingDues||0;ue&&(ue.textContent=L(_e)),ge&&(ge.textContent=z.summary?.pendingStudentsCount?`${z.summary.pendingStudentsCount} students with dues`:"Pending recovery");const xe=document.getElementById("dash-kpi-total-seats"),ve=document.getElementById("dash-kpi-total-seats-sub");xe&&(xe.textContent=`${C} Desks`),ve&&(ve.textContent=`${B} Occupied, ${M.maintenance||0} Maintenance`);const be=document.getElementById("dash-kpi-renewals-week"),he=document.getElementById("dash-kpi-renewals-sub"),O=D.filter(t=>{if(t.daysRemaining!==void 0)return t.daysRemaining>=0&&t.daysRemaining<=7;if(t.expiryDate){const n=(new Date(t.expiryDate)-new Date)/864e5;return n>=0&&n<=7}return!1});be&&(be.textContent=`${O.length} students`),he&&(he.textContent=`${pe} expiring today / tomorrow`),window._dashWABlast=async()=>{if(!O.length){window.Toast&&S.info("No renewals due this week");return}let t="Study Library";try{t=(await f.get("/api/settings"))?.data?.businessProfile?.businessName||t}catch{}for(const n of O){const r=(n.phone||"").replace(/[^0-9]/g,"");if(!r||r.length<10)continue;const x=r.length===10?"91"+r:r,u=n.expiryDate?new Date(n.expiryDate).toLocaleDateString("en-IN"):"soon",s=`Hi ${n.name}! \u{1F44B}
Your library membership expires on *${u}*.
Please renew to continue your studies. \u{1F4DA}

\u2014 ${t}`;window.open(`https://wa.me/${x}?text=${encodeURIComponent(s)}`,"_blank"),await new Promise(d=>setTimeout(d,700))}window.Toast&&S.success(`\u{1F4F2} Opened ${O.length} WA links`)};const me=document.getElementById("dash-gauge-arc"),ye=document.getElementById("dash-gauge-pct"),fe=document.getElementById("dash-kpi-occ-value"),we=document.getElementById("dash-kpi-occ-sub");me&&me.setAttribute("stroke-dasharray",`${_}, 100`),ye&&(ye.textContent=`${_}%`),fe&&(fe.textContent=`${B}/${C}`),we&&(we.textContent=`Occupied / Total \u2022 ${_}% full`);const ke=document.getElementById("dash-kpi-behavior-alerts"),Ee=document.getElementById("dash-kpi-alerts-sub"),$e=(F.expired||0)+(F.inactive||0);ke&&(ke.textContent=$e),Ee&&(Ee.textContent=`${F.expired||0} expired + ${F.inactive||0} inactive`),document.querySelectorAll(".stat-card-value").forEach(t=>{const n=t.textContent?.replace(/[₹,\s]/g,"").replace(/[^0-9.]/g,""),r=parseFloat(n);if(!isNaN(r)&&r>0&&!t.dataset.animated){t.dataset.animated="1";const x=t.textContent.startsWith("\u20B9")?"\u20B9":"",u=t.textContent.replace(/[₹0-9,.\s]/g,"").trim();let s=0;const d=r/30,g=setInterval(()=>{s=Math.min(s+d,r),t.textContent=x+Math.round(s).toLocaleString("en-IN")+(u?" "+u:""),s>=r&&clearInterval(g)},20)}});const Fe=document.getElementById("dash-present-today"),Ae=document.getElementById("dash-available-seats"),Ce=document.getElementById("dash-active-subs");Fe&&(Fe.textContent=`${N.stats?.totalPresent||N.stats?.totalCheckedIn||0} Students`),Ae&&(Ae.textContent=`${ae} Available`),Ce&&(Ce.textContent=`${F.active??0}`);const K=document.getElementById("dash-expiring-container");if(K&&(D.length===0?K.innerHTML=`
          ...
          <div class="d-flex flex-column divide-y">
            ${D.slice(0,6).map(t=>{const n=t.expiryDate?new Date(t.expiryDate):null,r=n&&!isNaN(n.getTime()),x=t.daysRemaining??(r?Math.ceil((n-new Date)/(1e3*60*60*24)):0),u=r?n.toLocaleDateString("en-IN"):"N/A",s=(t.phone||"").replace(/[^0-9]/g,""),d=encodeURIComponent(`Hi ${t.name}, friendly reminder from Study Library: Your desk membership expires on ${u}. Please renew to retain your seat!`),g=s?`https://api.whatsapp.com/send?phone=${s.length===10?"91"+s:s}&text=${d}`:"#";return`
                <div class="p-3 d-flex justify-content-between align-items-center" style="border-bottom: 1px solid var(--color-divider, rgba(255,255,255,0.06));">
                  ...
                  <div class="d-flex align-items-center gap-2">
                    <span class="badge ${x<=2?"badge-danger":"badge-warning"}" style="font-size: 11px;">
                      ${x<=0?"Expires Today":`${x}d left`}
                    </span>
                    ${s?`
        ...
        `),document.getElementById("chart-revenue-trend-canvas")){const t=Array.isArray(z.trend)&&z.trend.length>0?z.trend.slice(-14):[{date:"Day 1",amount:3200},{date:"Day 3",amount:4800},{date:"Day 6",amount:2900},{date:"Day 9",amount:6200},{date:"Day 12",amount:5100},{date:"Day 15",amount:7400},{date:"Today",amount:T.todayRevenue||4500}],n=t.map(s=>{if(!s.date)return"";if(s.date.includes("-")){const d=s.date.split("-");return`${d[2]}/${d[1]}`}return s.date}),r=t.map(s=>s.amount||0),x=r.reduce((s,d)=>s+d,0),u=document.getElementById("dash-trend-total");u&&(u.textContent=L(x)),H.lineChart("chart-revenue-trend-canvas",{labels:n,data:r,color:"#0984e3",fill:!0,title:"Daily Collection (\u20B9)"})}if(document.getElementById("chart-shift-occupancy-canvas")){const t=Array.isArray(P.shiftStats)?P.shiftStats:Array.isArray(P)?P:[];let n=[],r=[];const x=["#6c5ce7","#00b894","#0984e3","#fdcb6e","#e17055","#a29bfe"];t.length>0?(n=t.map(g=>g.name||g.code),r=t.map(g=>g.enrolledStudents??g.enrolled??0)):(n=["Morning","Evening","Full Day","Night"],r=[8,12,16,4]);const u=r.reduce((g,E)=>g+E,0),s=document.getElementById("dash-shift-total");s&&(s.textContent=`${u} Students`),H.doughnutChart("chart-shift-occupancy-canvas",{labels:u>0?n:["No Students Enrolled"],data:u>0?r:[1],colors:u>0?x:["rgba(148, 163, 184, 0.2)"],title:"Shift Distribution"});const d=document.getElementById("chart-shift-legend");d&&(u>0?d.innerHTML=n.map((g,E)=>`
            <span style="display: inline-flex; align-items: center; gap: 4px;">
              <span style="width: 10px; height: 10px; border-radius: 50%; background: ${x[E%x.length]}; display: inline-block;"></span>
              ${A(g)}: <strong>${r[E]}</strong>
            </span>
          `).join(""):d.innerHTML='<span class="text-muted small">No students assigned to shifts yet</span>')}if(document.getElementById("chart-exam-stats-canvas")){const t={"UPSC / IAS":0,"SSC / CGL":0,"Banking / IBPS":0,"State PSC":0,"NEET / JEE":0,"Defence / NDA":0,Other:0};Array.isArray(J)&&J.length>0&&J.forEach(d=>{Array.isArray(d.targetExams)&&d.targetExams.length>0&&d.targetExams.forEach(g=>{const E=String(g).trim();if(t[E]!==void 0)t[E]++;else{let Se=!1;for(const Q of Object.keys(t))if(E.toLowerCase().includes(Q.toLowerCase())||Q.toLowerCase().includes(E.toLowerCase())){t[Q]++,Se=!0;break}Se||t.Other++}})});const n=Object.values(t).reduce((d,g)=>d+g,0),r=Object.keys(t).filter(d=>t[d]>0),x=r.map(d=>t[d]),u=["#6c5ce7","#00cec9","#fdcb6e","#e84393","#0984e3","#00b894","#636e72"];H.doughnutChart("chart-exam-stats-canvas",{labels:n>0?r:["No Exam Records"],data:n>0?x:[1],colors:n>0?u:["rgba(148, 163, 184, 0.2)"],title:"Exam Preparation"});const s=document.getElementById("chart-exam-legend");s&&(n>0?s.innerHTML=r.map((d,g)=>`
            <span style="display: inline-flex; align-items: center; gap: 4px;">
              <span style="width: 10px; height: 10px; border-radius: 50%; background: ${u[g%u.length]}; display: inline-block;"></span>
              ${A(d)}: <strong>${x[g]}</strong>
            </span>
