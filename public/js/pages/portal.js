import P from"../api.js";import{Toast as S,Modal as re,Confirm as Ze,Loading as he,escapeHTML as o,copyToClipboard as Re}from"../ui.js";import{SmartFormatters as we}from"../utils/smartFormatters.js";import"../i18n.js";import{previewAdmissionFormPDF as qe,buildReceiptHTML as Qe,printReceiptDocument as Je}from"../pdfGenerator.js";import{PushNotifications as ke}from"../utils/pushNotifications.js";import{renderHeatmap as Xe}from"../utils/attendanceHeatmap.js";import{MediaFieldPicker as et}from"../mediaStudio.js";import{SmartIntelligence as Oe}from"../utils/smartIntelligence.js";import{PaymentStudio as De}from"../paymentStudio.js";async function Ne(){const n=document.createElement("div");n.className="page-container",n.style.cssText="width: 100%; max-width: 100%; box-sizing: border-box; padding-bottom: 3rem;",n.innerHTML=`
  ...
  `;try{const[g,E]=await Promise.all([P.get("/api/student-portal/dashboard"),P.get("/api/student-portal/config").catch(()=>null)]);if(!g.success||!g.data)throw new Error(g.message);const t=E?.data?.features||{};window.store||(window.store={}),window.store.portalFeatures=t;let p=null;try{const I=await P.get(`/api/attendance/analytics/${g.data.student._id}`);I.success&&(p=I.data)}catch(I){console.warn("Analytics fetch error:",I)}je(n,g.data,p),tt(n,t),setTimeout(async()=>{let I=n.querySelector("#portal-year-heatmap");if(!I){const T=n.querySelector('[data-section="analytics"]')||n.querySelector("#portal-analytics-section")||n.querySelector(".portal-analytics-card .card-body");T&&(I=document.createElement("div"),I.id="portal-year-heatmap",I.style.cssText="margin-top:18px;padding-top:14px;border-top:1px solid var(--color-border,rgba(255,255,255,0.08));",T.appendChild(I))}if(I&&g.data?.student?._id)try{await Xe(I,g.data.student._id,new Date().getFullYear(),{compact:!1})}catch{I.innerHTML='<div class="text-muted small text-center p-2">Full attendance calendar unavailable</div>'}},200)}catch(g){n.innerHTML=`
    ...
    <div class="portal-container">
      ...
      ">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; position: relative; z-index: 1; flex-wrap: wrap; gap: 10px;">
          <div>
            ...
            <div style="font-size: 0.78rem; opacity: 0.92; margin-top: 2px; font-weight: 600; color: #ffffff;">
              Shift: <span style="color: #a7f3d0; font-weight: 700;">${o(Ee)}</span> \u2022 Plan: <span style="color: #c7d2fe; font-weight: 700;">${o(ie)}</span>
            </div>
          </div>
        ...
        <div style="margin-top: 14px; background: rgba(0,0,0,0.3); height: 6px; border-radius: 4px; overflow: hidden; position: relative; z-index: 1;">
          <div style="height: 100%; width: ${Math.max(5,Math.min(100,I/30*100))}%; background: linear-gradient(90deg, #34d399, #a7f3d0); border-radius: 4px;"></div>
        </div>
        ...
        <div class="quick-stats-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 180px), 1fr)); gap: 8px; margin-bottom: 1.25rem;">
          <div class="card p-3" style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md);">
            ...
            <div style="font-size: 1.4rem; font-weight: 800; color: var(--color-warning); margin-top: 2px;">
              ${E?.currentStreak||t.studyStreakDays||0} Days
            </div>
            <div style="font-size: 0.70rem; color: var(--color-text-muted);">Best Streak: ${E?.longestStreak||1} days</div>
          </div>
          <div class="card p-3" style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md);">
            ...
            <div style="font-size: 0.70rem; color: var(--color-text-muted);">${E?.totalDaysPresent||1} days present this month</div>
          </div>
          <div class="card p-3" style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md);">
        ...
        <div class="card mb-4 p-3 p-md-4" style="background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-lg);">
          <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 0.75rem; margin-bottom: 1.25rem; border-bottom: 1px solid var(--color-divider); padding-bottom: 0.75rem;">
            ...
            <div style="display: flex; gap: 6px; flex-wrap: wrap; align-items: center;">
              <span class="badge" style="background: rgba(16, 185, 129, 0.15); color: var(--color-success); font-weight: 700; font-size: 0.76rem; padding: 4px 8px; border-radius: 6px;">
                ${o(E?.peakStudyHours?.badge||"\u{1F305} Peak: 08:00 AM \u2013 02:00 PM")}
              </span>
              <span class="badge" style="background: rgba(245, 158, 11, 0.15); color: var(--color-warning); font-weight: 700; font-size: 0.76rem; padding: 4px 8px; border-radius: 6px;">
                \u{1F525} ${E?.currentStreak||0} Day Streak
              </span>
            </div>
          </div>
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 250px), 1fr)); gap: 1.25rem; align-items: center;">
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center; gap: 6px; padding: 12px 14px; background: var(--color-bg-secondary); border-radius: var(--radius-md); border: 1px solid var(--color-border);">
              ${Ge(E?.consistencyScore||0)}
              <div style="font-size: 0.82rem; color: var(--color-text-secondary); margin-top: 4px;">
                Avg: <strong>${o(E?.averageDailyDuration?.formatted||"0m")}</strong> / day
              </div>
              <div style="font-size: 0.75rem; color: var(--color-text-muted);">
                ${E?.totalDaysPresent||0} / 30 days present
              </div>
            </div>
            ...
            <div>
              ...
              ">
                ...
                <div style="font-size: 0.80rem; color: var(--color-text-primary); line-height: 1.35;">
                  <strong>AI Study Tip:</strong> ${o(E?.aiRecommendation||E?.aiStudyTip||"Consistency is the key to cracking competitive exams. Try regular study blocks every morning!")}
                </div>
              </div>
            </div>
          </div>
  ...
  `;const Se=n.querySelectorAll(".portal-tab-pill"),fe={overview:n.querySelector("#pane-portal-overview"),campus:n.querySelector("#pane-portal-campus"),receipts:n.querySelector("#pane-portal-receipts")},J=a=>{Se.forEach(r=>{const i=r.getAttribute("data-portal-tab")===a;r.classList.toggle("active",i),r.style.background=i?"var(--color-surface)":"transparent",r.style.color=i?"var(--color-primary)":"var(--color-text-secondary)",r.style.boxShadow=i?"0 2px 8px rgba(0, 0, 0, 0.12)":"none"}),Object.keys(fe).forEach(r=>{fe[r]&&(fe[r].style.display=r===a?"block":"none")}),a==="campus"&&typeof B=="function"&&B("notices")};Se.forEach(a=>{a.addEventListener("click",()=>{J(a.getAttribute("data-portal-tab"))})}),n.querySelector("#tile-portal-idcard")?.addEventListener("click",()=>{n.querySelector("#btn-portal-idcard")?.click()}),n.querySelector("#tile-portal-renew")?.addEventListener("click",()=>{n.querySelector("#btn-portal-renew")?.click()}),n.querySelector("#tile-portal-receipts")?.addEventListener("click",()=>{J("receipts")}),n.querySelector("#tile-portal-notices")?.addEventListener("click",()=>{J("campus"),typeof B=="function"&&B("notices")}),n.querySelector("#tile-portal-holidays")?.addEventListener("click",()=>{J("campus"),typeof B=="function"&&B("holidays")}),n.querySelector("#tile-portal-lostfound")?.addEventListener("click",()=>{J("campus"),typeof B=="function"&&B("lostfound")}),n.querySelector("#tile-portal-feedback")?.addEventListener("click",()=>{J("campus"),typeof B=="function"&&B("feedback")}),n.querySelector("#admin-switch-student")?.addEventListener("change",async a=>{const r=a.target.value;n.innerHTML=`
            ...
            ">
              ...
              <div style="background: ${l.headerBg}; color: #fff; padding: 8px 10px; text-align: center; position: relative;">
                <div style="display: flex; align-items: center; justify-content: center; gap: 6px; margin-bottom: 2px;">
                  ${R?`<img src="${R}" style="width: 22px; height: 22px; border-radius: 4px; object-fit: contain; background: #fff;">`:""}
            ...
            ">
              ...
              <div style="background: ${l.headerBg}; color: #fff; padding: 8px 12px; display: flex; align-items: center; justify-content: space-between; gap: 8px;">
                <div style="display: flex; align-items: center; gap: 8px; min-width: 0;">
                  ${R?`<img src="${R}" style="width: 24px; height: 24px; border-radius: 4px; object-fit: contain; background: #fff; flex-shrink: 0;">`:""}
    ...
    `;const r=new re({title:"\u{1F464} My Admission Profile & Submitted Details",content:a,size:"lg"});r.show();try{let i=function(y){if(!y)return"";let c=String(y).trim();return c.includes("___")&&(c=c.replace(/___/g," / ")),c=c.replace(/_/g," "),c=c.replace(/([a-z])([A-Z])/g,"$1 $2"),c.split(" ").filter(Boolean).map(v=>v.charAt(0).toUpperCase()+v.slice(1).toLowerCase()).join(" ").replace(/\s*\/\s*/g," / ")},s=function(y,c){if(c==null||c==="")return'<span class="text-muted small">Not provided</span>';if(y.type==="star_rating"){const v=parseInt(c,10)||5;return`<span style="color: #f59e0b; font-size: 1.1rem;">${"\u2605".repeat(v)}${"\u2606".repeat(Math.max(0,5-v))}</span> <strong class="ms-1">(${v}/5)</strong>`}if(y.type==="checkbox"||y.type==="terms_checkbox"||y.type==="consent_checkbox")return c===!0||c==="true"||c==="on"||c===1?'<span class="badge badge-success">\u2705 Yes / Agreed</span>':'<span class="badge badge-secondary">\u274C No</span>';if(y.type==="photo_upload"||y.type==="file"||y.fieldName?.toLowerCase().includes("image")){const v=String(c).startsWith("data:image")||String(c).startsWith("/")?String(c):`/${c}`;return`
        ...
        <div class="mb-4" style="background: var(--color-bg-secondary); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 1.25rem;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 200px), 1fr)); gap: 14px; font-size: 0.88rem;">
            ...
            <div><span class="text-muted d-block small">Date of Birth</span><strong>${o(R)}</strong></div>
            ...
            <div><span class="text-muted d-block small">Emergency Contact</span><strong>${o(F)} (${o(V)})</strong></div>
      ...
      `;const Y={personal:"\u{1F464}",academic:"\u{1F3AF}",plan:"\u23F0",payment:"\u{1F4B3}",seat:"\u{1FA91}",contact:"\u{1F4CD}",kyc:"\u{1FAAA}",parent:"\u{1F468}\u200D\u{1F469}\u200D\u{1F467}",vehicle:"\u{1F697}",transport:"\u{1F6B2}",custom:"\u{1F4CB}",additional:"\u{1F4DD}",other:"\u{1F4DD}"},l=[];if(le.length>0){le.forEach(v=>{const N=pe.filter(K=>K.section===v.name).sort((K,Be)=>K.order-Be.order);N.length>0&&l.push({name:v.name,label:v.label||i(v.name),icon:v.icon&&v.icon.length<=4?v.icon:Y[v.name]||"\u{1F4CB}",fields:N})});const y=new Set(l.flatMap(v=>v.fields.map(N=>N.key))),c=pe.filter(v=>!y.has(v.key));(c.length>0||ce)&&l.push({name:"additional",label:"Additional Information & Preferences",icon:"\u{1F4DD}",fields:c.sort((v,N)=>v.order-N.order),remarks:ce})}else(pe.length>0||ce)&&l.push({name:"additional",label:"Additional Information & Preferences",icon:"\u{1F4DD}",fields:pe.sort((y,c)=>y.order-c.order),remarks:ce});l.forEach(y=>{se+=`
          <div class="mb-4" style="background: var(--color-bg-secondary); border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: 1.25rem;">
            <div style="font-weight: 700; font-size: 1rem; color: var(--color-primary); margin-bottom: 14px; display: flex; align-items: center; gap: 8px;">
    ...
    `,new re({title:"\u{1F4BA} Request Desk / Seat Transfer",content:a,size:"md"}).show();const r=a.querySelector("#sc-branch"),i=a.querySelector("#sc-target-seat"),s=a.querySelector("#sc-vacant-badge");let u=[],f=[];try{const[$,m]=await Promise.all([P.get("/api/student-portal/branches").catch(()=>P.get("/api/branches/public-list")).catch(()=>({data:[]})),P.get("/api/student-portal/available-seats").catch(()=>P.get("/api/seats/public-available")).catch(()=>({data:[]}))]);u=Array.isArray($.data)?$.data:$.data?.branches||[],f=Array.isArray(m.data)?m.data:m.data?.seats||[],u.length>0?r.innerHTML=u.map((d,R)=>`
        ...
        `).join("")}catch{$.innerHTML='<p class="text-danger small text-center">Failed to load history</p>'}}a.querySelector("#portal-sc-form").onsubmit=async $=>{$.preventDefault();const m=r.value,d=u.find(F=>String(F._id)===String(m)),R=d?d.name:"",j=i.value,G=i.options[i.selectedIndex],H=G&&G.dataset?.num||"",q=a.querySelector("#sc-zone").value,x=a.querySelector("#sc-reason").value.trim();try{await P.post("/api/student-portal/seat-change",{targetBranch:m,targetBranchName:R,targetSeat:j,targetSeatNumber:H,preferredZone:q,reason:x}),S.success("Seat transfer request submitted to branch manager!"),a.querySelector("#sc-reason").value="",D()}catch(F){S.error(F.message||"Failed to submit request")}},D()};n.querySelectorAll("#btn-portal-seat-change, #tile-portal-seat-change").forEach(a=>a.addEventListener("click",M));const $e=async()=>{const a=document.createElement("div");a.innerHTML=`
    ...
    `;const r=new re({title:"\u{1F381} Student Referral Studio & Rewards",content:a,size:"md"});r.show();try{const i=await P.get("/api/student-portal/referral-stats");if(!i.success)throw new Error(i.message);const{referralCode:s,referralCredits:u,totalReferralsCount:f,config:h,referrals:D=[]}=i.data,$=`${window.location.origin}/register?ref=${encodeURIComponent(s)}`,m=encodeURIComponent(`Hey! I study at ${p.businessName||"the study library"}. Use my referral code *${s}* to get \u20B9${h?.refereeRewardAmount||100} instant discount on your admission! Register here: ${$}`);a.innerHTML=`
      ...
      `,a.querySelector("#btn-copy-ref-code")?.addEventListener("click",d=>{Re(s,d.currentTarget)}),a.querySelector("#btn-copy-ref-link")?.addEventListener("click",d=>{Re($,d.currentTarget)}),a.querySelector("#btn-save-custom-code")?.addEventListener("click",async()=>{const d=a.querySelector("#custom-code-input"),R=d.value.trim().toUpperCase();if(!R){S.error("Please enter a custom code");return}try{const j=await P.put("/api/student-portal/custom-referral-code",{code:R});j.success?(S.success(j.message),a.querySelector("#display-ref-code").textContent=R,d.value=""):S.error(j.message)}catch(j){S.error(j.message||"Failed to update code")}}),a.querySelector("#portal-ref-form")?.addEventListener("submit",async d=>{d.preventDefault();const R=a.querySelector("#ref-name").value.trim(),j=a.querySelector("#ref-phone").value.trim(),G=a.querySelector("#ref-notes").value.trim();try{const H=await P.post("/api/student-portal/referral",{refereeName:R,refereePhone:j,targetExam:G});H.success?(S.success("Friend referral submitted! Thank you!"),r.close()):S.error(H.message)}catch(H){S.error(H.message||"Failed to submit referral")}})}catch(i){a.innerHTML=`<div class="text-danger p-4 text-center">Failed to load referral studio: ${o(i.message)}</div>`}};n.querySelectorAll("#btn-portal-referral, #tile-portal-referral").forEach(a=>a.addEventListener("click",$e));const ne=n.querySelectorAll(".btn-campus-tab"),O=n.querySelector("#campus-tab-content-container");async function B(a){if(ne.forEach(r=>{r.dataset.tab===a?r.className="btn btn-primary btn-campus-tab active":r.className="btn btn-outline-secondary btn-campus-tab"}),!!O){if(O.innerHTML='<div class="text-center p-4 text-muted"><div class="loading-spinner mb-2" style="margin: 0 auto;"></div>Loading...</div>',a==="notices")try{const r=(await P.get("/api/student-portal/announcements")).data||[];if(r.length===0){O.innerHTML='<div class="text-center p-4 text-muted">No campus circulars or notice board alerts at this time.</div>';return}O.innerHTML=`
    ...
    `;const r=new re({title:"\u{1F4AC} Student Feedback & Helpdesk",content:a,size:"md"});r.show(),a.querySelector("#form-submit-feedback").onsubmit=async i=>{i.preventDefault();const s=a.querySelector("#fb-category").value,u=a.querySelector("#fb-rating").value,f=a.querySelector("#fb-message").value.trim();try{const h=await P.post("/api/student-portal/feedback",{category:s,rating:u,message:f});h.success?(S.success("Feedback submitted! Thank you!"),r.close(),B("feedback")):S.error(h.message)}catch(h){S.error(h.message||"Failed to submit feedback")}}}ne.forEach(a=>{a.addEventListener("click",()=>{B(a.dataset.tab)})});function ve(a){const r=document.getElementById("student-campus-hub-card");r&&(r.scrollIntoView({behavior:"smooth",block:"center"}),r.style.boxShadow="0 0 0 3px rgba(99, 102, 241, 0.4)",setTimeout(()=>{r.style.boxShadow=""},1500),B(a))}n.querySelector("#btn-portal-notices")?.addEventListener("click",()=>ve("notices")),n.querySelector("#btn-portal-holidays")?.addEventListener("click",()=>ve("holidays")),n.querySelector("#btn-portal-lostfound")?.addEventListener("click",()=>ve("lostfound")),n.querySelector("#btn-portal-feedback")?.addEventListener("click",()=>ve("feedback")),B("notices"),n.querySelector("#btn-portal-receipts-jump")?.addEventListener("click",()=>{const a=document.getElementById("student-receipts-card");a&&(a.scrollIntoView({behavior:"smooth",block:"center"}),a.style.boxShadow="0 0 0 3px rgba(99, 102, 241, 0.4)",setTimeout(()=>{a.style.boxShadow=""},1500))}),n.querySelectorAll(".btn-view-receipt").forEach(a=>{a.addEventListener("click",async()=>{try{const r=JSON.parse(a.dataset.receipt),i=r.receiptNumber||"REC";let s=window.store?.settings?.receipt;if(!s||!s.activeTemplate)try{const f=await P.get("/api/settings/receipt-config").catch(()=>null);f?.success&&f?.data&&(s=f.data)}catch{}const u=`
        ...
        `,r()},r=function(){De.attachEventListeners(f),f.querySelectorAll(".btn-portal-pm").forEach(x=>{x.addEventListener("click",()=>{const F=x.dataset.mode;F&&F!==u&&(u=F,a())})});const h=f.querySelector("#renewal-plan-select"),D=f.querySelector("#renewal-shift-select");async function $(){const x=h?h.value:"",F=D?D.value:"";try{const z=await P.get(`/api/student-portal/renewal-quote?planId=${x}&shiftId=${F}&applyWallet=false`);z.success&&z.data&&(s=z.data,a())}catch(z){console.warn("Failed to calculate renewal quote:",z)}}h&&h.addEventListener("change",$),D&&D.addEventListener("change",$);let m=null;f.querySelectorAll(".btn-renewal-intent").forEach(x=>{x.onclick=F=>{F.preventDefault();const z=x.dataset.app,V=s.upiId||"7276969070@upi",W=s.businessName||"Study Library";m=`UPI_REN_${(t.phone||"STU").slice(-4)}_${Date.now().toString().slice(-6)}`;const A=f.querySelector("#renewal-utr-input");A&&(A.value=m);const k=f.querySelector("#renewal-auto-status");k&&(k.style.display="block",k.innerHTML=`
              ...
              `);const C=`pa=${encodeURIComponent(V)}&pn=${encodeURIComponent(W)}&am=${s.totalPayable}&cu=INR&tn=${encodeURIComponent("Library Membership Renewal")}&tr=${encodeURIComponent(m)}`;let U=`upi://pay?${C}`;z==="gpay"?U=`gpay://upi/pay?${C}`:z==="phonepe"?U=`phonepe://pay?${C}`:z==="paytm"&&(U=`paytmmp://pay?${C}`),window.location.href=U}}),f.querySelectorAll(".btn-portal-copy-bank").forEach(x=>{x.onclick=F=>{F.preventDefault();const z=x.dataset.copy;z&&navigator.clipboard.writeText(z).then(()=>{const V=x.textContent;x.textContent="\u2705",setTimeout(()=>{x.textContent=V},1500)}).catch(()=>{})}});const d=f.querySelector("#btn-portal-copy-all-bank");d&&(d.onclick=x=>{x.preventDefault();const F=`Bank: ${s.bankDetails?.bankName||"HDFC Bank"}
A/C No: ${s.bankDetails?.accountNumber||"50200012345678"}
IFSC: ${s.bankDetails?.ifscCode||"HDFC0000123"}
Beneficiary: ${s.bankDetails?.accountHolderName||s.businessName||"Study Library"}`;navigator.clipboard.writeText(F).then(()=>{d.textContent="\u2705 Copied All!",setTimeout(()=>{d.textContent="\u{1F4CB} Copy All"},1500)}).catch(()=>{})}),f.querySelectorAll(".btn-renewal-bank-intent").forEach(x=>{x.onclick=F=>{F.preventDefault();const z=x.dataset.bank;m=`BNK_REN_${(t.phone||"STU").slice(-4)}_${Date.now().toString().slice(-6)}`;const V=`${s.bankDetails?.accountNumber||"50200012345678"}`;navigator.clipboard.writeText(V).catch(()=>{});const W=f.querySelector("#renewal-utr-input");W&&(W.value=m);const A=f.querySelector("#renewal-nb-auto-status");A&&(A.style.display="block",A.innerHTML=`
  ...
  `}function We(n){return!n||n.length===0?'<div class="text-muted small text-center p-3">No attendance records found for the past 30 days.</div>':`
    <div>
      <div style="display: flex; flex-wrap: wrap; gap: 6px; align-items: center; justify-content: flex-start; padding: 4px 0;">
        ${n.map(g=>{const E=g.minutes||0,t=(E/60).toFixed(1),p=new Date(g.date),I=isNaN(p.getTime())?g.date:p.toLocaleDateString("en-IN",{month:"short",day:"numeric",weekday:"short"});let T="rgba(148, 163, 184, 0.15)",L="rgba(148, 163, 184, 0.25)",e="Absent";return g.status==="absent"||E===0?(T="rgba(148, 163, 184, 0.15)",L="rgba(148, 163, 184, 0.25)",e="Absent"):E<120?(T="#0e4429",L="#006d32",e=`${E} mins (${g.status})`):E<240?(T="#006d32",L="#26a641",e=`${t} hrs (${g.status})`):E<360?(T="#26a641",L="#39d353",e=`${t} hrs (${g.status})`):(T="#39d353",L="#2ea043",e=`${t} hrs (${g.status})`),`
      <div
        title="${`${I}: ${e}${g.checkIn?` [${g.checkIn} - ${g.checkOut||"Active"}]`:""}`}"
        style="
          width: 22px;
          height: 22px;
          border-radius: 4px;
  ...
  `}function ze(n){return new Promise(g=>{if(!n||typeof n!="string")return g(null);const E=n.trim();if(!E)return g(null);const t=Array.from(document.querySelectorAll("img")).find(T=>T.complete&&T.naturalWidth>0&&(T.src===E||T.getAttribute("src")===E||T.src.endsWith(E)));if(t)return g(t);let p=E;!p.startsWith("http")&&!p.startsWith("data:")&&!p.startsWith("blob:")&&!p.startsWith("/")&&(p="/"+p);const I=new Image;!p.startsWith("data:")&&!p.startsWith("blob:")&&(I.crossOrigin="anonymous"),I.onload=()=>g(I),I.onerror=()=>{if(I.crossOrigin){const T=new Image;T.onload=()=>g(T),T.onerror=()=>g(null),T.src=p}else g(null)},I.src=p})}async function Ye(n,g={},E="S",t="02",p="Study Plan",I="Not Set",T={}){S.info("\u{1F3A8} Generating 1080p Ultra-HD Mobile Pass Wallpaper...");const L=document.createElement("canvas");L.width=1080,L.height=1920;const e=L.getContext("2d"),ge=n.photo||n.avatar||n.profilePhoto||n.selfie||window.store?.user?.photo||window.store?.user?.avatar||document.querySelector("#sp-avatar-img")?.src||document.querySelector(".portal-profile-avatar img")?.src||"",ae=g.logo||g.logoUrl||window.store?.profile?.logo||window.store?.settings?.businessProfile?.logo||document.querySelector(".brand-logo img")?.src||"",Ce=g.stampImage||g.stampImageUrl||window.store?.profile?.stampImage||window.store?.settings?.businessProfile?.stampImage||"";let te=T.shiftName||n.shift?.name||n.shift?.timing||n.shift||n.plan?.shift||"Full Day";typeof te=="string"&&(te.toLowerCase()==="fullday"?te="Full Day":te=te.replace(/\b\w/g,k=>k.toUpperCase()));const Ue=te;let ie=p||n.plan?.name||"Study Plan";typeof ie=="string"&&(ie=ie.replace(/\b\w/g,k=>k.toUpperCase()));const Ve=ie,Ee=T.phone||n.phone||n.mobile||"-",be=T.bloodGroup||n.bloodGroup||"",He=n.studentId||n.enrollmentNo||"STU-MEMBER",Fe=n.admissionDate?new Date(n.admissionDate).toLocaleDateString("en-IN"):new Date().toLocaleDateString("en-IN"),Ie=`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(n.studentId||n.phone||n._id||"STUDENT")}&margin=2&bgcolor=ffffff`,[Q,Te,Se,fe]=await Promise.all([ze(ge),ze(ae),ze(Ce),ze(Ie)]),J=e.createLinearGradient(0,0,1080,1920);J.addColorStop(0,"#0a0d18"),J.addColorStop(.3,"#13182e"),J.addColorStop(.7,"#181534"),J.addColorStop(1,"#090b14"),e.fillStyle=J,e.fillRect(0,0,1080,1920),e.save(),e.beginPath(),e.arc(180,240,450,0,Math.PI*2),e.fillStyle="rgba(79, 70, 229, 0.12)",e.fill(),e.beginPath(),e.arc(900,1680,400,0,Math.PI*2),e.fillStyle="rgba(16, 185, 129, 0.08)",e.fill(),e.restore(),e.textAlign="center",e.fillStyle="rgba(255, 255, 255, 0.7)",e.font='600 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(`\u{1F4F1} OFFLINE DIGITAL PASS \u2022 ${(g.businessName||"STUDY LIBRARY").toUpperCase()}`,540,75);const w=80,b=120,_=920,Z=1660,M=36;e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(w,b,_,Z,M):(e.moveTo(w+M,b),e.arcTo(w+_,b,w+_,b+Z,M),e.arcTo(w+_,b+Z,w,b+Z,M),e.arcTo(w,b+Z,w,b,M),e.arcTo(w,b,w+_,b,M)),e.closePath(),e.fillStyle="#ffffff",e.shadowColor="rgba(0, 0, 0, 0.45)",e.shadowBlur=50,e.shadowOffsetY=24,e.fill(),e.strokeStyle="#0f172a",e.lineWidth=4,e.stroke(),e.restore(),e.save(),e.setLineDash([16,10]),e.beginPath(),typeof e.roundRect=="function"&&e.roundRect(w-14,b-14,_+28,Z+28,M+10),e.strokeStyle="rgba(255, 255, 255, 0.45)",e.lineWidth=2.5,e.stroke(),e.restore(),e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(w,b,_,240,[M,M,0,0]):(e.moveTo(w,b+M),e.arcTo(w,b,w+M,b,M),e.lineTo(w+_-M,b),e.arcTo(w+_,b,w+_,b+M,M),e.lineTo(w+_,b+240),e.lineTo(w,b+240)),e.closePath(),e.clip();const $e=e.createLinearGradient(w,b,w+_,b+240);if($e.addColorStop(0,"#4f46e5"),$e.addColorStop(1,"#6366f1"),e.fillStyle=$e,e.fillRect(w,b,_,240),Te){const k=w+36,C=b+36;e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(k,C,64,64,12):e.rect(k,C,64,64),e.fillStyle="#ffffff",e.fill(),e.clip(),e.drawImage(Te,k,C,64,64),e.restore(),e.textAlign="left",e.fillStyle="#ffffff",e.font='800 36px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(g.businessName||"Study Library",w+115,b+68),e.fillStyle="rgba(255, 255, 255, 0.88)",e.font='500 22px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(g.tagline||"Silence, Focus and Success",w+115,b+102)}else e.textAlign="center",e.fillStyle="#ffffff",e.font='800 38px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(g.businessName||"STUDY LIBRARY",540,b+70),e.fillStyle="rgba(255, 255, 255, 0.88)",e.font='500 22px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(g.tagline||"Silence, Focus and Success",540,b+106);e.restore();const ne=540,O=b+240,B=100;if(e.save(),e.beginPath(),e.arc(ne,O,B+10,0,Math.PI*2),e.fillStyle="#ffffff",e.shadowColor="rgba(0, 0, 0, 0.15)",e.shadowBlur=20,e.shadowOffsetY=6,e.fill(),e.beginPath(),e.arc(ne,O,B+4,0,Math.PI*2),e.fillStyle="#4f46e5",e.fill(),e.beginPath(),e.arc(ne,O,B,0,Math.PI*2),e.closePath(),e.clip(),Q){const k=Q.naturalWidth||Q.width||200,C=Q.naturalHeight||Q.height||200,U=Math.min(k,C),X=(k-U)/2,oe=(C-U)/2;e.drawImage(Q,X,oe,U,U,ne-B,O-B,B*2,B*2)}else e.fillStyle="#eef2ff",e.fillRect(ne-B,O-B,B*2,B*2),e.fillStyle="#4f46e5",e.font='800 76px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.textAlign="center",e.textBaseline="middle",e.fillText(E,ne,O);e.restore(),e.textBaseline="alphabetic";const de=(n.name||"Student Member").trim();e.textAlign="center",e.fillStyle="#0f172a";let ye=44;e.font=`800 ${ye}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`;let ve=e.measureText(de).width,a=[];if(ve>760){const k=de.split(" ");if(k.length>=2){const C=Math.ceil(k.length/2),U=k.slice(0,C).join(" "),X=k.slice(C).join(" ");a=[U,X],ye=38}else a=[de],ye=34}else a=[de];e.font=`800 ${ye}px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`;let r=b+390;a.length>1?(a.forEach((k,C)=>{e.fillText(k,540,r+C*46)}),r+=(a.length-1)*46):e.fillText(a[0],540,r);const i=r+22,s=He;e.font="800 26px monospace";const u=e.measureText(s).width+36,f=46;let h=0;const D=!!be,$=`\u{1FA78} ${be}`;D&&(e.font='800 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',h=e.measureText($).width+28);let m=540-(u+(D?12+h:0))/2;if(e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(m,i,u,f,10):e.rect(m,i,u,f),e.fillStyle="#eef2ff",e.fill(),e.strokeStyle="#c7d2fe",e.lineWidth=1.5,e.stroke(),e.fillStyle="#4338ca",e.font="800 26px monospace",e.textAlign="center",e.fillText(s,m+u/2,i+32),e.restore(),D){const k=m+u+12;e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(k,i,h,f,10):e.rect(k,i,h,f),e.fillStyle="rgba(220, 38, 38, 0.1)",e.fill(),e.strokeStyle="rgba(220, 38, 38, 0.25)",e.lineWidth=1.5,e.stroke(),e.fillStyle="#dc2626",e.font='800 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.textAlign="center",e.fillText($,k+h/2,i+32),e.restore()}const d=w+44,R=i+68,j=_-88,G=490;e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(d,R,j,G,20):e.rect(d,R,j,G),e.fillStyle="#f8fafc",e.fill(),e.strokeStyle="#e2e8f0",e.lineWidth=2,e.stroke(),e.restore();const H=[{label:"Assigned Desk / Seat:",val:t,color:"#4f46e5",bold:!0},{label:"Shift Timing:",val:Ue,color:"#0f172a",bold:!0},{label:"Study Plan:",val:p,color:"#334155",bold:!1},{label:"Contact Phone:",val:Ee,color:"#334155",bold:!1},{label:"Valid Until:",val:I,color:"#dc2626",bold:!0}];H.forEach((k,C)=>{const U=R+64+C*92;e.textAlign="left",e.fillStyle="#64748b",e.font='600 28px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(k.label,d+32,U),e.textAlign="right",e.fillStyle=k.color,e.font=`${k.bold?"800":"600"} 30px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`,e.fillText(k.val,d+j-32,U),C<H.length-1&&(e.beginPath(),e.moveTo(d+24,U+30),e.lineTo(d+j-24,U+30),e.strokeStyle="#e2e8f0",e.lineWidth=1.5,e.stroke())});const q=R+G+36,x=240,F=w+70;fe&&(e.save(),e.beginPath(),typeof e.roundRect=="function"&&e.roundRect(F-8,q-8,x+16,x+16,12),e.fillStyle="#ffffff",e.fill(),e.strokeStyle="#cbd5e1",e.lineWidth=2,e.stroke(),e.drawImage(fe,F,q,x,x),e.restore(),e.textAlign="center",e.fillStyle="#4f46e5",e.font="800 20px monospace",e.fillText("SCAN TO VERIFY PASS",F+x/2,q+x+30));const z=w+_-340,V=q+10;Se?(e.save(),e.drawImage(Se,z+40,V,180,180),e.restore()):(e.save(),e.translate(z+130,V+80),e.rotate(-.06),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(-100,-45,200,90,8):e.rect(-100,-45,200,90),e.strokeStyle="#059669",e.lineWidth=3,e.stroke(),e.fillStyle="#059669",e.textAlign="center",e.font='800 24px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText("OFFICIAL SEAL",0,-10),e.font='700 20px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText("PAID & VERIFIED",0,20),e.restore()),e.beginPath(),e.moveTo(z+10,V+210),e.lineTo(z+250,V+210),e.strokeStyle="#94a3b8",e.lineWidth=2,e.stroke(),e.textAlign="center",e.fillStyle="#64748b",e.font='600 20px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText("Authorized Signatory",z+130,V+238);const W=b+Z-68;e.save(),e.beginPath(),typeof e.roundRect=="function"?e.roundRect(w,W,_,68,[0,0,M,M]):(e.moveTo(w,W),e.lineTo(w+_,W),e.lineTo(w+_,b+Z-M),e.arcTo(w+_,b+Z,w+_-M,b+Z,M),e.lineTo(w+M,b+Z),e.arcTo(w,b+Z,w,b+Z-M,M),e.lineTo(w,W)),e.closePath(),e.fillStyle="#f8fafc",e.fill(),e.strokeStyle="#e2e8f0",e.lineWidth=1,e.stroke(),e.restore(),e.textAlign="center",e.fillStyle="#64748b",e.font='600 22px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText(`Issued: ${Fe} \u2022 Helpline: ${g.phone||"+91 98765 43210"} \u2022 Non-Transferable`,540,W+42),e.textAlign="center",e.fillStyle="rgba(255, 255, 255, 0.45)",e.font='500 20px -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif',e.fillText("Carry on phone lockscreen for instant entry \u2022 Anti-Tamper Digital Token",540,1870);const A=document.createElement("a");A.download=`${de.replace(/\s+/g,"_")}_Mobile_ID_Pass_1080x1920.png`,A.href=L.toDataURL("image/png"),document.body.appendChild(A),A.click(),A.remove(),S.success("\u{1F4F1} 1080x1920px Mobile ID Pass Wallpaper downloaded successfully!")}export{Ye as download1080pMobileIDPass,Ne as render,Ge as renderGaugeScoreSvg,We as renderHeatmapGridHtml};
